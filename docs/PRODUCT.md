...
| **专题视觉** | 地图 / 关系 / 器物·礼仪 / 时间线：按场景选用；**定稿** 图文分离 + NotebookLM 式结构→版式→填格（§19.14.12–14）；行程半屏已接 layout JSON |
...
| Web 消费 | `MapStoryMode`：有 layout 时同屏 **①叙事弧 · ②路径 · ③站序事实 · ④多格 · ⑤讲解条**；字在 UI/SVG |
| 分享 | **上图下文信息图**（`knowledge_infographic_share`）：干净母题 + 站序事实面板；走完一程「分享信息图」 |

##### 流水线（工程）

```
map_tours（结构源：站序 / ref / note / ask_seed）
  + CURATED 策展（arc / happen / chips / must_see / vignette）
  → compile_knowledge_layout.py
  → knowledge_layout@1
  → API → MapStoryMode（path_diagram + beat_grid + explain_bar）
...
| ID | 用途 |
|----|------|
| `path_grid_explain` | 行程默认：示意路径 + 多格 + 讲解条（旷野 / 保罗 / 加利利） |
...
| **§19.14.14 信息图闭环** | **2026-09-16** | 上图下文分享 + `POST /knowledge-layouts/from-scripture` + MapStory 多区块同屏 |
