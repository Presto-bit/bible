...
class StyledElement {
  ...
  bool matches(dom.Element element, String selector) {
    try {
      // html >=0.15.7 removed top-level qs.matches; emulate via parent query.
      final parent = element.parentNode;
      if (parent == null) {
        return qs.querySelector(element, selector) == element;
      }
      return qs.querySelectorAll(parent, selector).contains(element);
    } catch (_) {
      return false;
    }
  }
...
extension DeepCopy on ListQueue<Counter> {
  ListQueue<Counter> deepCopy() {
    return ListQueue.from(map((counter) {
