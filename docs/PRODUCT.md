...
- **首页入口：** 今日推荐第 4 坑为 **探索**（**固定插图底** + 当日随机专题短名/导语；点进该专题全屏手稿；从首页进入后返回落在 `/knowledge` 专题列表，不回首页）；成长区去重主题卡，「开始祷告」下移成长区。专题列表 `/knowledge`（**1 行 2 卡**、按 `generated_at` 降序）仍可从列表进入。  
- **续读：** 全屏手稿本机记住页码；再次打开续读；列表卡显示「续 · N」。  
...
- **列表筛选：** 全部 / 行程 / 笔记。  
- **管理员新建：** `/knowledge/new`（需管理员）先选 **写文字 / 传图片 / 传音频 / 传视频**，再进分型表单；支持存草稿、文字流可生成纸页竖图；笔记卡可「下架」。  
- **明确不做：** 笔记**定时发布**（本期不做）。
