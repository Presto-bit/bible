...
> **关于版本号：** 本文档此前的 `v1.1 ~ v3.1.1` 为 **设计迭代轮次**（评审打磨的内部编号），现统一 **冻结为产品 v1.0 正式版**。下方各 `vN.N 变更摘要` 与文末修订记录 **保留为设计演进史**，便于追溯；功能范围以 §7 功能清单 + §13 版本规划（Phase 1/2/3）为准。

## v1.0 正式版 · 范围与待优化清单（2026-06-29）

**1.0 已冻结的核心范围：** 5 Tab 闭环（首页 / 圣经 / 小爱 / 发现 / 我的）；CNV+KJV 经包与沉浸阅读；读经计划 + 祷告计划；半屏接力释经 + 小爱自由对话（RAG + 脚标引用 + 额度合规）；阅读报告与成就；社交 = 共读群（结构化打卡/任务流 + emoji，无聊天，**受邀/创建制**）+ 好友分享（仅点赞，无 IM）。**（v1.1 移除公开群广场/拼群找群）**

**待优化清单（Phase 2+ 候选，按主题归并）：**

| 类别 | 待优化项 | 状态/说明 | 优先级 |
|------|----------|-----------|--------|
| **上线前阻断项** | 新译本文本入库（EPUB→DB）、首批 3–5 本注释入库 | 内容就位才能跑通 RAG（§14）；**AI 选型 ✅**（DeepSeek + DashScope，Key 已配）；**版权 ✅ 直接执行**（授权由业务方落实）；**域名 ✅** `prestoai.cn`；**上云 3 决策 ✅** 定稿（§2.4.2） | **P0** |
| **上线前阻断项** | 品牌名与中文字标定稿 | §19.11 待定 | **P0** |
| **阅读体验补齐** | KJV 对照模式、交叉引用、资源指南、原文 Sheet（Strong's） | Phase 2（§5.2、§13.2） | P1 |
| **小爱补齐** | 对照/原文模式、对话历史长期化、查经/讲道结构化输出、预填模板表统一配置 | Phase 2（§5.3、§7） | P1 |
| **数据与同步** | 笔记/进度/AI 历史云同步、游客合并稳态、阅读报告 Dashboard 完整图表 | Phase 1 末 / Phase 2（§8、§9） | P1 |
| **推送与召回** | 每日提醒、断签温和召回、群消息 **聚合推送**（每日 ≤2 条，可静音） | Phase 2（§10、§5.4.4 F1） | P1 |
| **祷告计划深化** | 首页今日卡支持 **读经/祷告动态切换**；祷告打卡并入报告 | **30 天 ACTS 逐日内容 ✅ 已生成**（`data/plans/prayer_acts_30.json`）；首页动态切换/报告并入待接（§5.1） | P1 |
| **群 · 体验** | H 组（群节奏/本地时区/补卡/宽容连续/计划日历）；J 组（轮值领读、观察者角色） | 已入 §5.4.4 规格，未排期 | P1/P2 |
| **群 · 冷启动** | G3 个人计划一键成群、G4 新手破冰首卡（G1 广场/G2 拼群已做 demo） | §5.4.4 G 组 | P1 |
| **群 · 后续主题（未采纳留档）** | I 内容形态（领读音频/语音打卡/证道→任务）、K 个人×群打通（统一收件箱/一次打卡多处生效/打卡隐私）、L 留存见证（代祷应允/纪念册/健康分） | 评审留档，未纳入 1.0 | P2 |
| **社交其它** | 共享计划进度同步、收藏经节复习卡片 | §7 P2 | P2 |
| **内容安全** | 打卡/任务文本审核与敏感词、举报、群主删消息的 **后端流水线** | 规格已定，需工程化 | P1 |
| **数据度量** | 新功能埋点补齐：群广场/拼群、祷告计划、缓存秒回、半屏接力漏斗 | 指标口径待定（§17.5） | P1 |
| **可访问性/国际化** | 大字号无障碍、语音打卡转文字、多译本与多语言管理 | 未系统规划 | P2 |
| **视觉精美化** | 完成卷微动画 | **每日经文/专题低饱和插画 ✅ 已生成**（20 张主题 SVG，`data/illustrations/`，见 §19.12）；微动画待做 | P2 |
...
| **来源脚标** | 句末或段末 **上标 [n]**；点击脚标 → 展开该条 **注释摘录**（再次点击收起） |
...
- **可折叠（v2.8）：** 每次小爱回答须带 **「依据注释摘录」** 区块（默认折叠），或显式 **「资料库中暂无，以下为一般性说明」**
- 无资料 →「资料库中暂无」
- 每条 AI 可反馈「有帮助 / 不准确」
- 首次使用 + 设置页免责声明
...
- 每次回答 **固定**「依据注释摘录」或「暂无依据」说明  
- 设置：**保守回答** / **多观点并列**（默认并列）  
- 详见 §6.7

#### 20.5.2 牧养咨询免责声明

**焦虑**、**悲伤** 发现专题页固定短句：「**本 App 不能替代牧养咨询**……」详见 §5.4。
...
**定位：** 小爱每天 **主动推一个默想问题/追问**，形成对话式灵修；这是 **AI 能力（非社交）**，不违背「无聊天」原则。
