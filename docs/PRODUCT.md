...
- 中文标题：白底条 + 系统黑体叠字（与样张策略一致）；不依赖生图模型写字。  
- **免责说明（新图必有，不回刷历史资产）**：叠在画面 **右下角** 小字 **「释义说明，仅供参考」**（可读、不抢主标题；勿再居中铺底栏）。半屏/全屏 UI 与导出图同文案。标明示意非考古复原、非经文替代。  
...
卡片简报（含 refs + scripture_text 经文原文摘录 + must_see/must_not）
    ...
    → 叠：部位标注（可选）+ 标题 + 出处；**右下角**「释义说明，仅供参考」（仅新图；不回刷历史）+ 压 WebP 双档 → 入库
```
