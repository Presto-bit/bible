...
_MARKDOWN_OUTPUT = (
    "【Markdown 规范】\n"
    "须一次性写完全部规定 ### 小节与足够 - 要点后再停笔，不要留半成品。\n"
...
_CONTINUITY = (
    "读者与你已有上文对话（可能含半屏 OIA 四步快懂：摘要/背景/解释/回应）。"
...
_MARKDOWN_OUTPUT_COMPACT = (
    ...
    "- ### 摘要 — 观察：1 句完整话（约 35–55 字），直接说经文核心；主谓完整、自然收束；"
    "禁止残缺起笔（如「住一个误解」），勿用半句话开头。\n"
    "- ### 经文背景 — 背景：当时处境、前后文或同卷主线的衔接（≤60 字，勿重复摘要）。\n"
    "- ### 经文解释 — 解释：关键字句在当时语境下的原意（≤80 字，不写大段考据）。\n"
...
def depth_format_guide(profile: DepthProfile, scene_id: str, verse_span: int = 1) -> str:
    ...
    if profile.depth in ("oia_standard", "oia_deep"):
        ...
        if profile.depth == "oia_deep" and "段落脉络" in profile.sections:
            extra = "\n多节经：在 ### 经文背景 与 ### 经文解释 之间插入 ### 段落脉络（3–4 条）。"
        return (
    ...
    if has_prior_turns:
        ...
        if question and is_full_oia_request(question):
            system_parts.append(
                ...
                "补充 1–2 个上文未展开的细节，勿重复摘要/背景/解释/回应。\n"
            )
    if narrow:
        system_parts.append("\n")
