...
> 版本：**v1.0 + v1.2 社交 + 活动运营 + 我的身份页 + 双端 §24（Flutter×H5 白名单 2026-08-05）**  
> 更新日期：2026-09-17  
> 平台：iOS（PWA）+ Android（Flutter + H5 白名单）+ Web
...
> **双端交付形态** 以 **[§24](#24-双端交付--ios-pwa--android-flutterh5-白名单2026-08-05-定稿)** 为准：iOS PWA + Android Flutter；**IM 等指定面 H5**；**Shorebird 仅壳热修**。  
> **「我的」Tab 主屏 IA / 头像上传 / 同行主卡** 以 **[§5.5（2026-07-30 定稿）](#55-我的--读经身份页2026-07-30-定稿)** 为准；与旧「纵向功能卡片列表」冲突时，**以本节为准**。  
> **活动运营（群定向首页活动、模板自配、多日日课、分享讨论）** 以独立规格为准：[`CAMPAIGN-OPS.md`](./CAMPAIGN-OPS.md)。

## 双端交付（2026-08-05）· 摘要

| 模块 | 定案 |
|------|------|
| **产品句** | 同一账号、同一五 Tab、同一业务能力；**iOS = Safari PWA**；**Android = Flutter 官网安装包** |
| **一致性** | **硬** = 事一致 + 账号/社交互通；**软** = 原生面可分端 UI；**H5 白名单与 iOS 同套 Web**（IM 高度一致） |
| **H5 白名单** | **发现/IM**、**活动/运营落地**、**部分我的二级**、**协议/帮助/反馈**、**部分设置 Web 页** |
| **Flutter 原生** | 启动/Tab 壳、**首页**、**圣经读经**、**小爱**、**我的主屏**、提醒/推送/深链/安装 |
| **IM** | **唯一实现 = Web**；iOS 直用；安卓发现 **嵌 H5**；**禁止** Flutter 再造 IM |
| **小爱** | **安卓 Flutter 原生**；与 PWA **事对齐**（规则/额度/锚定以后端为准）；UI 允许分端 |
| **Shorebird** | **仅壳/原生系统层热修**；**不是**业务发布平台；能 Web 就不 Shorebird，能 Shorebird 就不整包 |
| **安卓容器** | 白名单内 **System WebView**；禁止「无 Chrome 不可用」；TWA 非主路径 |

完整规格见 **[§24](#24-双端交付--ios-pwa--android-flutterh5-白名单2026-08-05-定稿)**。

## 人生主题删除 · 知识专题视觉化（2026-09-16）· 摘要

| 模块 | 定案 |
|------|------|
| **人生主题** | **从产品删除**：盼望 / 焦虑 / 祷告 / 家庭 / 工作 / 悲伤 / 信心 / 宽恕等宫格、专题页、搜索入口、色板入口（旧 §5.4.1 / §19.10）。深链 `/discover/topic/*` → `/search` |
| **保留** | 搜索页的 **知识专题**（地图行程 · 关系图 · 时间线 · 示意 diagram）；`data/topics/topics.json` 仅作 **主题→经文索引**（离线包名「主题经文索引」），**不是**人生主题产品 |
| **专题视觉** | 地图 / 关系 / 器物·礼仪 / 时间线：按场景选用；**定稿** 主交付 **密图**（版式引擎出字 + AI 填格，§19.14.15）；Explainer / 上图下文为次要（§19.14.12–14） |
| **扩库清单** | 适合图片说明的经文内容优先级见 **§19.14.9** |

## 圣经阅读 Chrome / 翻页 / 译本（2026-08-15 定稿修订）

> 覆盖并废止下文仍写「3s idle 自动藏栏 / 沉浸 FAB 半透明 / 章内分页页码 / 译本不限多选」的旧句；工程以本节为准。

| 项 | 定案 |
|----|------|
| **藏栏触发** | **点按正文切换**顶栏 + 底栏 Tab；**不做** 3s 无操作自动藏栏 |
| **沉浸时 FAB** | 打卡 + 小爱 **整组消失**（与全屏/专注一致）；点屏恢复 chrome 后再显示 |
| **选中经文时 FAB** | **隐藏**（避免与选中条抢点） |
| **左右滑** | **换章**（非章内按屏分页）；章末继续滑 → **下一卷首章**；全书尽头轻 Toast |
| **极简页码 `3/12`** | **不做**（随章内分页一并取消） |
| **译本勾选** | **最多 2 本**（主 + 对照）；顶栏摘要 `CNV+KJV` 形态；不做「N 个译本」 |

## 「我的」读经身份页（2026-07-30）· 摘要

| 模块 | 定案 |
|------|------|
| **定位** | **读经身份页**（我是谁 · 读到哪 · 留下了什么）；**非**功能面板 / 九宫格；首页管今日任务，「我的」管身份与资产 |
| **UI** | **与首页同构**：复用 soft card / `home-list-row` 语汇、圆角阴影 token；不另起视觉体系 |
| **主屏结构** | Hero（头像+昵称+**签名**）→ **同行主卡**（整卡进回顾）→ **我的足迹 2×2** → **常用 ≤4** → 设置仍在齿轮 Sheet |
| **同行主卡** | 本月已读天 · 本周/今日分钟 · 通读进度；**无「查看回顾」文案**；**整卡可点** → `/report`；数字口径与回顾页一致 |
| **足迹 2×2** | 笔记 / 书架 / 成就 / 进行中的事（计划优先，否则通读%）；满空同高；预览 ≤1 行 |
| **头像** | **默认系统预设**；**支持用户上传**（裁圆·压缩·可恢复预设）；社交回落：有自定义图用自定义，否则 `avatar_id` |
| **签名** | **就是签名**（≤15 字）；**不是**「今日一句 / 每日经文」 |
| **推进** | P0 主卡+骨架 2×2+签名厘清 → P1 真实预览+常用收敛+细节打磨 → P2 头像上传全链路+回流角标等；详见 §5.5.8 |

完整规格与推进方案见 **[§5.5](#55-我的--读经身份页2026-07-30-定稿)**。

## 活动运营规格（2026-07-23）· 摘要

| 模块 | 定案 |
|------|------|
| **入口** | **管理后台 → 活动运营**（仅平台超管；PC 配置）；**不做**群内活动配置入口 |
| **权限** | **本期：仅平台超管可配置**；受众可多选群或全站；后续再开放群主 |
| **曝光** | 今日推荐最多 3 卡（默认第 1 · 主卡）；**不挂 Hero B**；按成员过滤 |
| **落地** | **强模板 + 积木自配**；新建：场景→模板后进入**完整编辑配置**（不全量铺开模板、无三步向导） |
| **场景覆盖** | 读经日课、代祷、聚会/RSVP、迎新、节期、见证、背诵、服事招募、习惯挑战（见规格 §8；不含意见治理与宣教外展专题） |
| **多日** | **N 日日课**控件/模板：一份多日资料在单个活动内配置并引导阅读 |
| **互动** | 点赞、评论、代祷、报名等；可深链「去群里讨论」 |
| **分享** | 分享卡片 + 受众鉴权（非成员不泄漏正文） |

完整规格、分期与**成熟产品优化（§20）**见 **[`CAMPAIGN-OPS.md`](./CAMPAIGN-OPS.md)**。
...
| **社交交付（v1.2）** | §23 全量：双 Tab、申请制、群闲聊+管理员、成熟 IM、30 天清理、审核台与异端工单 | **规格已定稿**；工程按 §23.10 分批达标上线；**实现面 = Web IM（§24）** | **P0** |
| **双端交付（§24）** | iOS PWA + Android Flutter；H5 白名单承载 IM/活动等；原生读经/小爱/首页；Shorebird 壳热修 | **§24 定稿（2026-08-05）**；工程按 §24.7 | **P0** |
...
| **活动运营** | 群定向首页活动、模板自配、N 日日课、点赞评论、分享 | 规格 [`CAMPAIGN-OPS.md`](./CAMPAIGN-OPS.md)；**Web MVP～V1.2 已落地**（不含 Flutter；Hero 并入待做） | P1 |
| **主机与运维安全** | SSH 密钥-only、密钥轮换、Web 劫持监测、异地备份 | 生产运维（非本功能规格） | P0 |
| **可访问性/国际化** | 大字号、多译本与多语言 | 未系统规划 | P2 |
| **视觉精美化** | 完成卷微动画等 | 插画已有；微动画待做 | P2 |
| **我的 · 读经身份页** | 同行主卡整卡可点、足迹 2×2、常用收敛、头像上传、同构 UI 打磨 | **规格已定稿 §5.5**；按 §5.5.8 P0→P2 推进 | **P0/P1** |
...
1. [产品概述](#1-产品概述)
2. [产品原则](#2-产品原则)（含 [§2.6 设计一致性规则](#26-产品设计一致性规则仓库固化--2026-07-30)）
3. [目标用户](#3-目标用户)
4. [信息架构与导航](#4-信息架构与导航)
...
24. [双端交付 · iOS PWA × Android Flutter × H5 白名单（2026-08-05 定稿）](#24-双端交付--ios-pwa--android-flutterh5-白名单2026-08-05-定稿)

> 活动运营完整规格见独立文档：[`CAMPAIGN-OPS.md`](./CAMPAIGN-OPS.md)（含多日日课、分享、模板与控件库）。
...
| 模块 | 变更 |
|------|------|
| **搜索** | 圣经顶栏/目录 **搜索图标** 打开 **搜索页**；下方为 **知识专题**（地图/关系/时间线/示意）。**人生主题宫格已删除（2026-09-16）** |
...
| 优化 | 说明 | 状态 |
|------|------|------|
| **读经身份页（2026-07-30）** | 同行主卡整卡可点、足迹 2×2、常用≤4、签名非今日一句、头像预设+上传、同构 UI | **定稿 §5.5**；推进见 §5.5.8 |
| 成就 **分组** | 「全部」页按连续/卷/计划/笔记/习惯分组 | 路线图 |
| **快完成 tie-break** | 同进度：计划里程碑 > 卷完成 > 笔记 | 路线图 |
| 阅读卡片 **下钻** | 点周/月/年卡 → 下钻 **日视图** 并定位区间 | 报告内保留 |
| 书卷弹窗 **可点卷** | 目录式卷卡点击 → 章列表或继续读 | 保留 |
| **`2+15%` 说明** | 帮助文案解释遍数格式 | 路线图 |
| **读经计划入口** | ✅ v2.0 已纳入：＋ 菜单 → 计划弹窗 | 已完成 |

> **主屏：** 原「纵向成长/常用软卡片栈」由 §5.5.1 取代；报告与徽章能力改由主卡 / 足迹消费。
...
| 客户端 | **§24**：**iOS = Safari PWA**；**Android = Flutter 官网包**；**IM/活动等 = H5 白名单**；首页/读经/小爱/我的主屏 = Flutter 原生；**Shorebird 仅壳热修** |
...
### 2.5 全局手势词典（Web 产品面 · 含 iOS PWA 与 Android 嵌 Web · 2026-07-30）

> **目标：** 全 App 统一语义，避免「同一滑动干不同事」。**圣经 Tab 阅读翻页**细则见 §5.2。  
> **原则：** 手势只做导航与刷新；每个手势须有可见后备（‹ / 按钮）；`prefers-reduced-motion` 可降级动画。

| 手势 | 含义 | 适用 | 明确不做 |
|------|------|------|----------|
| **左缘右滑**（约 24px） | 返回上一层 | 二级路由（发现子页、群/私信、报告、设置子页、知识库、计划/祷告、活动等） | 根 Tab；阅读器中部翻页区 |
| **顶下拉** | **仅刷新当前面数据** | 首页；**发现 · 消息列表** | 「下拉进入某功能」；阅读器内下拉进小爱 |
| **中部左右滑** | 同级内容切换 | 阅读翻页（§5.2）；Chip / 卡轨横滑 | 根 Tab 左右切换 |
| **长按** | 次要操作 | 足迹分享、消息菜单 | 主路径唯一入口 |
| **列表行左滑** | 会话快捷操作 | 发现会话：置顶 / 免打扰 / 删除 | 聊天气泡左右滑切会话 |

**工程：** `useEdgeSwipeBack`；首页 / 发现下拉共用 `useHomePullRefresh`（发现关底 overscroll）。

### 2.6 产品设计一致性规则（仓库固化 · 2026-07-30）

> **效力：** 新增功能或改动体验时必须对齐。Cursor 侧同步为 `.cursor/rules/beiai-product-design.mdc`（`alwaysApply`）。社交细节以 **§23** 为准；视觉以 **§19** 为准。

#### 品牌与使命

| # | 规则 |
|---|------|
| 1 | 品牌名 **彼爱**；调性「安静读经，在话语中相遇」——温润克制，不做喧闹增长产品 |
| 2 | 使命：帮人 **读懂、坚持读、用得上**；差异化 = 读经 + **可溯源小爱（RAG）** + **共读关系**，不是通用 IM / 通用 ChatGPT |

#### 情感与文案

| # | 规则 |
|---|------|
| 3 | **被接纳 / 被理解 / 在进步**；陪伴感非竞赛：禁排行榜、公开时长攀比、「你落后了」guilt |
| 4 | 评审先问：强化「读」还是「刷」？ |
...
| # | 规则 |
|---|------|
| 5 | 5 Tab 锁定：首页今日 · 圣经沉浸 · 小爱深研 · 发现消息/关系 · 我的身份资产 |
| 6 | **一屏一事 / 唯一 accent**；继续阅读在圣经，不在首页主战场；「我的」禁九宫格/第二套任务流 |
| 7 | **读经勿扰**：圣经 Tab 无消息红点/横幅；底栏发现 **永不**未读角标 |
...
|---|------|
| 8 | **读经第一**；阅读像纸质圣经，非信息流 |
| 9 | 路径：边读 → 半屏快懂 → 问小爱；可溯源优先；默认锚定经节，禁永久单会话污染上下文 |

#### 发现与共读 IM（对齐 §23）

| # | 规则 |
|---|------|
| 10 | 发现 = 消息收件箱 + 通讯录；**无好友动态、无公开群广场** |
| 11 | **共读轻 IM**：成熟基础体验要有；禁通话/红包/朋友圈/群已读到人等重社交 |
| 12 | 申请制好友、受邀创建群；打卡/任务/经文卡一等公民；消息 **30 天**物理删除并诚实告知 |
| 13 | 交互可对齐微信熟悉路径，视觉仍服从彼爱静穆体系；**IM 唯一实现 = Web**（iOS PWA 直用，Android 发现 **嵌 H5**）；**禁止** Flutter 再造 IM（§24） |
| 13a | **双端一致**：硬 = 事一致；H5 白名单与 iOS **同套 UI**；原生面 UI 可分端；系统差异仅白名单（§24.4） |
| 13b | 需求/发版打标：`surface: h5 \| flutter \| shell`，`release: web \| shorebird \| full_apk`；**Shorebird 禁止承载业务** |

#### 视觉、交互与数据

| # | 规则 |
|---|------|
| 14 | 静穆温润、纸感 soft card、橄榄绿 accent 克制；经文衬线 / UI 无衬线 |
| 15 | 发现不做首页问候行、不做横滑卡轨；手势须有可见后备（§2.5）；少打断、可降级动效 |
| 16 | 云优先 + 离线可用（缓存/outbox）；离线不假装在线；边界文案诚实 |

#### 改功能三道门（必过）

| 门 | 问题 | 不通过则 |
|----|------|----------|
| **使命** | 是否帮助读懂 / 坚持 / 用上 / 共读？ | 砍或后置 |
| **边界** | 是否撞 §23「明确不做」或圣经勿扰？ | 禁止上线 |
| **气质** | 是否静穆、零 guilt、一屏一事？ | 改交互/文案再上 |

#### 2.6.1 今日温习与提醒（2026-07-30 对齐）

| 能力 | 旧观感 | 定稿 |
|------|--------|------|
| **今日温习**（原「今日 5 题 / 闯关」） | 正确率环、闯关关卡、竞赛语感 | **五道轻问**；错题优先；完成页鼓舞不审判；答后可「去读经文」；主题/关卡收入「更多」 |
| **答题交互** | 翻卡 + 自动跳题 | **同屏反馈**；用户点「下一题」；顶栏安全区可退；左缘滑退 |
| **提醒与勿扰** | 设置堆叠仪表盘 | **一屏一事**：主卡=每日读经提醒（默认关）；次卡=**读经勿扰**（默认开）；消息摘要/群打卡晚间收入「可选」 |
| **入口** | 我的 · 常用（曾 overlay 直开） | 文案「今日温习」「提醒」；**统一进** `/challenge`、`/profile/reminders` |
| **明确不做** | — | 不做排行/连胜羞辱；底栏发现不加未读角标；圣经页不弹社交提示（勿扰）。**听读**见 `docs/AI_LISTEN_PRODUCT.md`（阅读页 AI 听经，非首页听书馆） |
...
| **发现** | **好友分享**、共读群、共享计划动态 | ~~人生主题宫格~~（已删除，2026-09-16） |
| **我的** | **身份 Hero**、同行主卡（→报告）、足迹 2×2、常用捷径、设置 Sheet | **今日任务流**、会话列表、经文正文、社交分享流、九宫格功能墙 |
...
│  ●○○○                                │
├─────────────────────────────────────┤
│  今日 12 分钟 · 本月已读 9 天    [›]  │  ← ③ 摘要主行（可附副行里程碑）
├─────────────────────────────────────┤  ← 折叠线以下
│  [同行/发现] 落点（与今日推荐去重） › │  ← B
│  [去年今日] …（可选记忆）         ›  │  ← C ≤1
│         · 已经到底了 ·               │  ← D
...
#### 首页内容丰富（v1.1 · 一 / 二 / 五；听经不在首页）

> **取舍：** 本轮仅落地三组（① 深化每日经文 hero、② 折叠线以下渐进内容、⑤ 成长钩子）；**首页不做听经入口**（听读在圣经阅读页，见 `docs/AI_LISTEN_PRODUCT.md`）。「快捷 chips、时段问候/今日一句祷告」等暂缓。整体仍守「静穆温润 / 一屏一事 / 唯一 accent」。
...
**⑤ 成长钩子 + 稳定落点 + 到底反馈**（2026-07-30 定稿 · 收口）

> **现行 IA：** 每日经文 → **今日推荐** → A/B/C/D。旧「个性化卡轨」线框仅作演进史，不以之为准。  
> **不动**每日经文与今日推荐槽位逻辑；B 与推荐区**去重**，不重复小组。

| 块 | 内容 | 规则 |
|----|------|------|
| **A 摘要** | **主行锁定**「今日 X 分钟 · 本月 Y 天」→ `/report`；**副行可选**里程碑（就快读完 > 月末足迹 > 年末生成） | 主行永不被替换；副行点进续读/报告/wrapped |
| **B 稳定落点** | 小组 / 同行 / 发现 | 推荐侧卡已露出群深链 → **禁止小组**，改同行或发现；侧卡已是好友/创建共读时允许小组；文案共用 `formatHomeGroupLine` |
| **C 记忆卡** | 去年今日优先；月末/年末（仅 Dec–Jan） | ≤1；副行已是就快读完时不出 C；**无 accent** |
| **D 到底** | 「· 已经到底了 ·」 | 不足一屏也露出；触底触觉**每会话 ≤1**；短页首屏不震 |
| **下拉刷新** | 顶弹性；成功轻提示「已更新」 | 横滑不抢；1.5s 频控；reduced-motion 降级 |

**就快读完算法：** 仅当前阅读卷；`1…current` 无空洞；`1≤ total-current ≤3`；跳转 `current+1`。有空洞则不出。

```
今日推荐（群信息以这里为准）
│  今日 12 分钟 · 本月已读 9 天    [›]  │  ← A 主行
│  马可福音还剩 2 章 · 读完它           │  ← A 副行（可选）
│  [同行] 今天 3 位好友已打卡       ›  │  ← B（有群且推荐已占群 → 不出小组）
│  [去年今日] 诗篇23 · 你划了线     ›  │  ← C 可选
│         · 已经到底了 ·               │  ← D
```

> **护栏：** 非红点、非催促；成长资产不进首页列表；底拉 ≠ 顶刷新；同屏情绪钩子（副行就快读完 / C）互斥。
...
| **多选** | 支持勾选；**最多 2 本**（主译本 + 对照）；至少保留 1 个 |
| **阅读页** | **逐节对照**：第一选中译本为主正文；第二本 **弱化行**（小一号、tertiary 色）跟在每节下方 |
| **顶栏摘要** | 1 本显示缩写；2 本显示 `CNV+KJV` 形态；**不做**「N 个译本」 |
...
| **对话** | 引号 + 可选首行缩进 |
| **诗歌** | 左缩进、行距略紧 |
| **对照** | 最多双译本逐节附属行，非第二套 UI |
| **沉浸** | **点按**隐藏顶栏 + 底栏 Tab；**小爱/打卡 FAB 整组消失**；半屏/选中/overlay 时见 **Chrome 状态机** |

目录页顶栏可保留 **搜索图标**（全经搜索入口），与阅读页一致。

#### 阅读翻页（v2.2 · 2026-08-15 修订为换章）

| 项 | 规格 |
|----|------|
| **手势（唯一翻页方式）** | **左滑** → 下一章；**右滑** → 上一章 |
| **无按钮** | **不提供** 上一页/下一页图标或文字按钮；避免打断沉浸 |
| 分页单位 | 以 **章** 为界；章内 **竖向滚动** 阅读（**不做** 章内按屏分页） |
| **章末自动衔接** | 章末再左滑 → **下一章**；若已是该卷末章 → **下一卷第 1 章**；章首再右滑 → 上一章 / 上一卷末章 |
| 全书边界 | 圣经末章 / 首章：轻 **弹性回弹** + 可选 Toast「已到尽头」 |
| 反馈 | **不做** 极简页码 `3/12` |
| 冲突 | 与系统返回手势共存：屏幕 **左缘 16dp** 保留系统返回；章内中部滑动换章 |
| 首次引导 | 首次进入阅读页 **一次性** 轻提示「左右滑动换章」（短暂消失，不再打扰） |
...
#### 沉浸模式 + AI 入口（v1.4 · v2.8 状态机 · 2026-08-15 修订）

| 元素 | 行为 |
|------|------|
| **顶栏** | 章节｜版本 + 搜索 + 字体 |
| **底栏 Tab** | **点按正文** → 与顶栏一并隐藏 / 恢复（**无** 3s idle 自动藏） |
| **AI FAB** | **右下角「小爱」浮标**（可带文案）；沉浸 / 全屏 / 专注时 **整组隐藏**（含打卡） |
| **触发** | **点按正文** → 顶栏 + 底栏 Tab 淡出或恢复 |
| **恢复** | **再次点按** → 顶栏立即显示；**半屏/overlay 打开时** 点内容区 **不** 误触恢复顶栏 |

**Chrome 状态机（v2.8 · 2026-08-15）：**

状态变量：`chromeHidden` · `verseSelected` · `halfSheetOpen` · `catalogOverlayOpen` · `noteSheetOpen` · `compareSheetOpen`

| 状态 | 顶栏 | 底栏 Tab | 小爱/打卡 FAB |
|------|:----:|:--------:|:-------------:|
| 默认阅读 | 显示 | 显示 | 显示 |
| 沉浸（点按藏栏） | 隐藏 | 隐藏 | **隐藏** |
| **半屏小爱打开** | 显示/恢复 | 可隐藏 | **隐藏** |
...
**优先级（互斥）：** `halfSheetOpen` > `verseSelected` > `catalogOverlayOpen` > `chromeHidden`
...
| **形态** | 选区上方 **浮动工具条** |
| **主条（6 项）** | **笔记** · **划线** · **复制** · **金句卡** · **对照** · **小爱**（安卓 / PWA 一致） |
| **笔记** | 打开 **想法 Sheet**，锚定选中经节（对应足迹「想法」；主条文案为「笔记」） |
| **划线** | 展开颜色点阵；已划线则 **取消划线**；沉浸模式隐藏 |
| **复制** | 复制 **含出处** |
| **金句卡** | 打开 **金句卡** 分享 Sheet |
| **对照** | 打开 **对照 Sheet**（多译本逐节，与 §5.2 对照阅读一致）；研经模式显示 |
| **小爱** | 带经节锚点 → **半屏小爱**（主路径）；深问通过半屏 **「去问小爱」** 接力 Tab |
| **消失** | 取消选区 / 点击空白 / 切换章页 → 工具条收起 |
| **与沉浸** | 选中时 **隐藏 FAB**；顶栏可保持隐藏；专注模式仅保留复制等低打扰项 |
...
v2.1 起 **选中弹出条** 为主交互；长按仍可作为 **扩大选区** 的系统手势。收藏 / 原文（Strong's）等扩展能力走阅读设置或其他入口，**不在主条展示**。
...
> **人生主题已删除（2026-09-16）。** 搜索页仅保留知识专题（地图/关系/时间线/示意）；色板 §19.10 归档；微计划不再挂人生主题 CTA。
...
### 5.4.1 搜索页 — 经文 + 知识专题（2026-09-16 修订）

> **废止（2026-09-16）：** 「人生主题宫格」（盼望 / 焦虑等）、人生主题专题页、占位「搜索经文、人生主题…」、§19.10 人生主题色板入口。下文旧稿若仍写人生主题，一律以本节与文首摘要为准。

**入口：** 圣经 Tab 顶栏/目录 **搜索图标** → 全屏搜索页。

**结构（自上而下）：**

1. **搜索框**（占位：**「搜索经文…」**）+ ‹ 返回  
2. **搜索历史**（显示全部 · 不折叠；长按可删）  
3. **动态区**（随输入：经文 / 笔记 / 资源）  
4. **知识专题入口**（地图行程 · 关系 · 时间线 · 示意；**无**人生处境宫格）
...
| 有输入无命中 | 空结果提示；知识专题入口可保留 |

**历史记录：** 记最终到达的实体（经文 ref 或知识专题 id），**不再**记 `专题·盼望` 类人生主题。

| 区块 | 规格 |
|------|------|
| **搜索框** | 占位：「搜索经文…」 |
| **知识专题** | 进入地图 / 关系图 / 时间线 / diagram；视觉主图见 **§19.14.8** |
| **返回** | ‹ 关闭搜索，回到圣经上下文 |

---

### 5.5 我的 — 读经身份页（2026-07-30 定稿）

**定位：** **读经身份页**——一眼看见「我是谁、读到哪、留下了什么」。  
**不做：** 功能九宫格、第二套首页任务流、会话列表、经文正文。  
**与首页分工：** 首页定「今天做什么」；「我的」定「我的故事与资产」。  
**UI 原则：** **与首页 / 全 App 同构**（soft card、圆角阴影、字号阶梯、Sheet）；不另起视觉体系、不换皮。

> **覆盖关系：** 本节 **主屏 IA、同行主卡、足迹 2×2、头像上传、签名语义、推进方案** 为现行定稿。下文「历史规格（v2.4 卡片列表等）」仅作演进参考；冲突时以 **2026-07-30 定稿** 为准。报告下钻、经文库、徽章插画等 **目的地能力** 仍有效，由足迹/主卡入口消费。
...
| 区块 | 职责 | 明确不做 |
|------|------|----------|
| **Hero** | 头像、昵称、**签名**；右上分享 + 设置 | 不当设置条；签名 **不是**「今日一句」 |
| **同行主卡** | 本月已读天 · 时长 · 通读环；**整卡**进 `/report` | **无**「查看回顾」文案或次按钮 |
| **我的足迹 2×2** | 个人内容/资产预览 | 不做带 `›` 的功能行列表 |
| **常用** | ≤4 个工具捷径 | 不重复足迹已有入口；不扩九宫格 |
| **设置 Sheet** | 账号 / 工具 / 关于 / 退出 | 主屏不上浮表单墙；Admin 深藏 |
...
| 项 | 规格 |
|----|------|
| **默认** | **系统预设插画**（现有 `PRESET_AVATARS` / 景致徽章式）；新用户随机或默认 id（如 `a1`） |
| **更换预设** | 点头像 → Sheet → **预设网格**（保留现交互） |
| **上传** | Sheet 提供 **「从相册选择」**；裁剪为正圆（或方图居中裁圆）→ **压缩**（建议长边 ≤512）→ 保存 |
| **恢复** | 提供「使用预设」回到 `avatar_id`，清除自定义图引用 |
| **同步** | 账号完备后随资料同步；未完备可先本机，完备后 push |
| **社交回落** | 有自定义头像 URL 则用之；否则回落 `avatar_id` 预设（群/私信/列表小图缓存，避免全端闪大图） |
| **审核** | 上传走既有媒体/内容安全策略；失败可回上一张 |

**昵称：** 可点编辑；失焦或确认保存；与现账号规则一致。

**签名（定稿）**

| 项 | 规格 |
|----|------|
| **语义** | **用户自定义签名**；**不是**「今日一句 / 每日经文 / 系统问候」 |
| **长度** | ≤15 字（`maxLength` + 字数） |
| **空态** | 「点击添加签名」 |
| **编辑** | 点签名即编辑、失焦保存；失败 toast，成功无打断 |
| **展示** | 仅 Hero 副文；主卡 **不复述** 签名 |

**账号未完备：** Hero 下 **一条细提示**（如「完善账号，换机也能同步」）→ 点开既有引导；**禁止**半页安全表单墙占主屏。

#### 5.5.3 同行主卡

| 项 | 规格 |
|----|------|
| **展示** | 「本月已读 N 天」+ 本周或今日分钟 + 通读进度环（轻量） |
| **口径** | 本月有效阅读自然日（`minutes>0 || chapters>0`），与首页摘要 / `/report` 的 `monthDays` **完全一致**；点进去数字对得上 |
| **交互** | **整卡可点** → `/report`（或阅读时长详情入口，与现报告路由对齐） |
| **文案** | **不出现**「查看回顾 ›」或任何次 CTA；按下态与首页行一致 |
| **去重** | 删除主屏独立「读经回顾」列表行；连续天数不在 Hero 元信息区重复刷屏（主卡为主） |

#### 5.5.4 我的足迹（2×2 预览）

| 格 | 预览规则（≤1 行） | 点击 | 空状态 |
|----|-------------------|------|--------|
| **笔记** | 最新一条截断，或数量 | → `/notes`（想法；库内保留划线 Tab） | 「写下第一句」 |
| **书架** | 上次阅读书名，或「N 本」 | → `/shelf` | 「打开书架」 |
| **成就** | 最新解锁缩略 +「已获 N」 | → 徽章 Sheet / 全部 | 「读经解锁」 |
| **进行中的事** | 有计划→计划进度%；否则通读%；再否则引导 | → 计划或读经旅程 | 「开始一段旅程」 |

| 护栏 | 说明 |
|------|------|
| **满空同高** | 四格固定高度，防文案把整排撑乱 |
| **性能** | 预览用 **本地/缓存摘要**；Tab 唤醒轻量刷新；禁止进页并行打满 4 个重接口 |
| **隐私** | 预览 **仅自己可见**；不做公开主页（若未来开放须单独开关） |
| **回流（P1+）** | 从读经/笔记/书架返回且有新内容时，对应格 **轻角标**；点进后清除；非红点催促 |
| **长按（可选 P2）** | 分享该条笔记；主路径仍是单击进入 |
| **划线入口** | 足迹不再单列划线格；读经选中条「划线」保留；笔记库内 **想法 / 划线** Tab 可达 |
| **去重** | 「我的」主屏 **不再** 在足迹下另挂书架大卡 / 封面横滑 |
...
|------|------|
| 今日温习 | → `/challenge` |
| 提醒 | → 提醒与勿扰（`/profile/reminders`） |
| 离线 | → 离线圣经下载 |
| （可选）外观 / 读经旅程 | 若通读已并入主卡，旅程可不占常用 |

**禁止**在常用中再挂：想法、成就、读经回顾（已由足迹/主卡承担）。

#### 5.5.6 设置 Sheet（保持）

账号、下载离线、知识库（若开）、推送、外观、许可、帮助与反馈、PWA、清除缓存、退出、版本；**管理后台**仅 admin 且置于底部。主屏不上浮。

#### 5.5.7 体验与性能护栏（定稿补充）

| 项 | 规格 |
|----|------|
| **弱网** | 先渲染本地缓存的主卡 + 足迹，再静默刷新；进页不白屏转圈 |
| **动效** | 连续天变化 / 新徽章可用一次轻反馈；禁止动效秀场 |
| **里程碑分享** | 同行 7/30/100 天等用现有分享卡 **偶发**触发；日常不放常驻分享条 |
| **年度/阶段回顾** | 能力挂在 `/report` 内章节；**不**在「我的」主屏再加一张卡 |
| **明确不做** | 会员/积分墙、首页式今日任务流、九宫格、签名改每日经文、主卡「查看回顾」文案 |

#### 5.5.8 推进方案（工程排期）

| 阶段 | 目标 | 范围 | 验收 |
|------|------|------|------|
| **P0** | 主屏从「功能列表」变为「身份页骨架」 | 同行主卡（整卡→报告、无回顾 CTA）；去掉重复回顾行；签名文案/空态厘清；足迹 **2×2 骨架**（可先数量/空态）；常用初收敛；同构 UI 间距对齐首页 | 主屏无「查看回顾」；点主卡进报告；2×2 四格在位 |
| **P1** | 足迹有内容感 + 细节打磨 | 2×2 **真实预览**（笔记/书架/成就/进行中）；满空同高；本地摘要刷新；常用最终 ≤4；账号未完备改细条；轻回流角标（可选） | 预览与目的地数据一致；无进页卡顿 |
| **P2** | 头像上传与增强 | 相册上传 · 裁圆 · 压缩 · 恢复预设 · 资料同步 · 社交回落小图；里程碑分享；长按足迹分享（可选） | 预设默认仍可用；上传后群/私信正确回落 |

**工程落点（Web 优先）：** `ProfileTab`、`Avatar` / 头像 Sheet、`globals.css` profile 区、报告路由深链；同步层扩展自定义头像字段（P2）。Flutter 随多端对齐跟进。

**建议落地顺序：** P0 改 IA（体感最大）→ P1 预览数据 → P2 上传链路。

---

#### 历史规格备查（v2.4 / v1.1 · 主屏卡片列表已废止）

> 以下保留演进史与仍有效的 **目的地** 规格（经文库、徽章、报告下钻）。**主屏不再使用「今日时长→成就→书卷→我的经文」纵向四卡。**

##### 顶栏结构（v2.4 · 已被 5.5.1 Hero 取代）
...
| 元素 | 说明 |
|------|------|
| **头像** | 点头像 → 设置 · 个人资料；可换预设头像（**现已支持上传，见 5.5.2**） |
| **名称 + 签名** | 名称下一行为 **签名**（自定义 ≤15 字；原「个人成长」改名） |
| **用户 ID** | 名称区显示「ID xxxxxxxx · 长按复制」+「ID 仅自己可见」（仅本人可见，用于加好友） |
| **右上 ⚙** | 进入设置（含个人资料、阅读、外观、账号等） |

##### 个人资料规则（v1.1 · 头像条款已被 5.5.2 覆盖）

**① 头像 — 预设为默认，支持上传（2026-07-30 修订）**

| 项 | 规格 |
|----|------|
| **来源** | **默认系统预设**；**支持用户上传自定义图片**（见 §5.5.2；需裁剪/压缩/同步/审核） |
| **初始** | 注册即 **随机分配** 一个预设头像 |
| **数量** | **共 50 款预设**（v1.1 由 12 扩至 50） |
| **更换** | Sheet 内 **预设网格** + **从相册选择**；选中态高亮描边；可恢复预设 |
...
| **用途** | 加好友的唯一标识：好友在 **首页 ＋ → 加好友** 中 **输入对方用户 ID** 发起申请 |

##### 卡片列表（自上而下 · v2.4 · **主屏已废止**）

| # | 卡片 | 主展示 | 点击行为 | 现状态 |
|---|------|--------|----------|--------|
| 1 | **今日时长** | 今日分钟数 + 本月已读天数摘要 | → 阅读时长详情 | **并入同行主卡** |
| 2 | **成就徽章** | 横滑徽章预览 | → 全部徽章 | **并入足迹 2×2** |
| 3 | **书卷进度** | 最近在读 UsageBar | → 66 卷弹窗 | **进度并主卡 / 旅程常用** |
| 4 | **我的经文** | 计数 + 预览 | → 我的经文库 | **并入足迹：笔记**（库内想法/划线） |

**删除（v2.4）：** ~~下次读什么~~（智能推荐改由首页计划与圣经继续阅读承担）。

> **v1.1 合并：** 原「收藏经文复习 + 最近笔记」两张单卡 **合并为一张「我的经文」入口**（详见下「我的经文库」）。足迹 2×2 为入口预览；库内体验不变。

##### 我的经文库（v1.1 · 合并「收藏 / 高亮 / 笔记」）

> **动机：** 旧两卡是被动的"单条左滑"。改为 **可检索、可整理、可操作** 的统一内容库——把"我的圣经沉淀"真正变得好查好用。

**入口：** 足迹「笔记」→ 经文库（默认想法；划线为库内 Tab，或 `/notes?tab=highlights`）。
...
| **入口** | 足迹 · 成就格 → 全部徽章列表 / Sheet |

##### 阅读数据下钻（v2.8）

| 点击 | 行为 |
|------|------|
| **同行主卡**（整卡） | → `/report` 或阅读详情；口径与主卡一致 |
| 「今日 X 分钟」/ 今日时长（报告内） | 阅读详情 · **日视图**，**高亮今天** |
...
| **圣经视觉卡** | 读经半屏/详情的示意卡（器物·场所·礼仪等）；规格与性能见 **§19.14** |
...
### 19.10 人生主题色板（已废止 · 2026-09-16）

> **废止：** 人生主题产品已删除；下表仅作历史色值归档，**新开发不得再引用**。知识专题视觉见 §19.14.8。

品牌主色 **橄榄绿 `#5B6B4F`** 仍有效。原各人生主题浅底 / accent（归档）：
...
~~Flutter：`discover_topics.dart`~~ — 随产品删除，勿再新增引用。
...
| 3 | ~~发现主题 8 色板~~ | ⛔ 人生主题已删；色板见 §19.10 归档 |
...
### 19.14 圣经视觉卡 · 内容边界与性能平衡（2026-09-16）

> **定位：** 读经过程中帮助「读懂」的示意卡（半屏 → 可放大），不是发现刷图流，也不是词典插图库。  
> **气质：** 静穆纸感、低饱和；叙事卡密度以「故事画在图里」为准（见 §19.14.7），**不**以「一屏一事」限死视觉卡；**不**抢经文主路径。  
> **样张目录：** `data/visual_cards/samples/`（生成预览；正式资产走云端，不打进 APK）。  
> **已删：** 人生主题（盼望/焦虑等）产品面；**不做**人生主题鸡汤海报式长卷。

#### 19.14.1 内容白名单（可做）

| 类别 | 例 | 说明 |
|------|----|------|
| **器物 / 圣所构件** | 约柜、灯台、陈设饼桌、铜祭坛、洗濯盆 | 可辨认示意，非考古级复原 |
| **场所 / 建筑远景** | 会幕外观、圣殿远景、帐幕院落 | 长方形会幕/石砌圣殿等符合经文概念 |
| **礼仪 / 叙事场景** | 过红海、过约旦河、献祭流程示意 | 远景剪影即可；**无**面孔近景 |
| **人物象征卡（窄）** | 仅「角色符号」级（如牧羊人剪影、鱼符号） | **禁止**逼真面容崇拜像 |

#### 19.14.2 内容黑名单（不做）

| 禁止 | 原因 |
|------|------|
| 词典式全书灌图 / 每节配图 | 刷感、包体与注意力双爆 |
| 抽象教义图解（三位一体几何等） | 易误导、难审 |
| 上帝 / 神显人形显现 | 信仰与产品边界 |
| 血腥、战争特写、惊悚 | 违「静穆」 |
| 模型乱写中文标题 | 汉字不稳定；**标题一律客户端/流水线叠字** |

#### 19.14.3 资产规格（交付，非生成原图）

生成可用较高分辨率预览；**入库必须压成双档**，禁止列表直解原图。

| 档位 | 用途 | 建议像素 | 格式 | 单张目标体积 |
|------|------|----------|------|----------------|
| **thumb** | 半屏卡、列表预览 | **720×360** 或 **960×480** | WebP | **80–150KB** |
| **full** | 全屏细看（点开再拉） | **1440×720** | WebP | **200–400KB** |

- 中文标题 / 锚点 / 免责：**一律后处理叠字**（模型不写字）；**提升规格见 §19.14.10**（废止「一律粗白底条」为唯一样式）。  
- **免责说明（新图必有，不回刷历史资产）**：叠在画面 **右下角** 小字 **「释义说明，仅供参考」**；半屏/全屏 UI 与导出图同文案。  
- 与 §19.12 矢量插画区分：视觉卡为 **位图示意**；主题 hero 仍优先 SVG。

#### 19.14.4 性能原则（怎么平衡）

**瓶颈不是「库里有多少张」，而是「同时解码进内存的有几张、每张多大」。**

| 杠杆 | 规格 |
|------|------|
| **库可以多** | 白名单内可扩到数百张；**云端按需**，**不**打进 APK / 首包 |
| **屏上要少** | 按书卷/章节懒加载；同一屏常驻大图 **≤1–2** 张 |
| **解码按屏宽** | Flutter：`cacheWidth` / `memCacheWidth` ≈ 屏宽 × DPR（沿用首页图策略）；Web：`srcset` + `loading="lazy"`，列表用 thumb、点开再 full |
| **本地缓存** | LRU（建议上限 **50–100MB** 或按天数淘汰）；弱网可只显标题+占位 |
| **禁止** | 列表预加载大量 full；每张都解 1440 原图像素 |

**粗算手感：** 300 张 × ~120KB thumb ≈ **36MB** 云端库可接受；若每张 ~1MB PNG 且列表同时解十余张 → 内存与卡顿会明显。

#### 19.14.5 一句话策略

**库可以多（云端 + 白名单），屏上要少（懒加载）；生成可以大，交付要小（WebP 双档）；解码按屏宽，不按原图像素。**

#### 19.14.6 生图流水线 · DeepSeek 写 prompt → 智谱出图

> **问题：** 直接用短中文喂生图模型，易出「通用沙漠帐篷 / 希腊神庙 / 峡谷浅滩」——缺经文形制与时代地理。  
> **定案：** **LLM（DeepSeek）只写生图 prompt**；**智谱图像模型只负责像素**；**中文标题后处理叠字**（§19.14.2）。不在用户读经主路径实时连环两模型（成本/延迟）；用于 **离线建库 / 运营补图**。  
> **默认图像模型：`cogview-3-flash`（CogView-3-Flash）**；备选 `cogView-4-250304` / `glm-image`（A/B 或专项时再用）。

##### 流水线

```
卡片简报（含 refs + scripture_text 经文原文摘录 + must_see/must_not）
    → DeepSeek（必须先摘动词语再写 prompt）→ JSON
    → 校验白名单 + 敏感词软化（避免智谱 1301）
    → 默认 cogview-3-flash 出图（画面内禁止任何文字）
    → 叠字引擎（§19.14.10：自适应对比 + 轻 scrim + L1–L4）→ 压 WebP 双档 → 入库
    →（App 内可选）下发 clean 图 + overlay_spec，运行时再叠
```

| 步 | 模型 / 工具 | 职责 |
|----|-------------|------|
| 1 | **DeepSeek**（现有 `deepseek-v4-flash`） | **强绑定经文原文** 写 prompt（见下）；英文空间描述为主 |
| 2 | **智谱图像（默认 `cogview-3-flash`）** | 仅图像；`watermark_enabled: false`（需账号开通）；默认 `size=1440x720` |
| 3 | 本地脚本 | 叠标注/标题/出处/**右下角免责**、WebP、元数据；**图面不印模型名** |

| 图像模型 | API `model` | 用途 |
|----------|-------------|------|
| **CogView-3-Flash（默认）** | `cogview-3-flash` | 建库默认；过红海等叙事神迹形制更稳、更快 |
| CogView-4 | `cogView-4-250304` | 备选对照 |
| GLM-Image | `glm-image` | 备选；人物/纸感可更好，宽画幅建议 `1728x960` 等官方枚举 |

##### 卡片简报（喂给 DeepSeek 的最小输入）

| 字段 | 例 | 必填 |
|------|----|------|
| `id` / `title_zh` / `category` | `red_sea_crossing` / 过红海 / 叙事 | ✓ |
| `refs` | `["出埃及记 14:21-29"]` | ✓ |
| **`scripture_text`** | 和合本（或当前主译本）**逐节原文摘录**，不少于叙事核心句 | ✓ **硬性** |
| `must_see` / `must_not` | 由原文关键词拆出的可见/禁止要素 | ✓ |
| `era_geo` | 时代与地理 | ✓ |

> **禁止**只给书卷章节号、不贴原文就让 DeepSeek「凭印象」写 prompt。原文缺则先从 `data/bible/.../verses.json` 抽取再调用。

##### DeepSeek「Prompt Writer」System（定稿摘要）

角色：圣经视觉顾问 + 文生图提示词工程师。只输出 **一个 JSON**，勿解释。

必须遵守：

1. **先读 `scripture_text`**：在 JSON 的 `verse_anchors` 中列出将可视化的经文原句（中文短引，可截断）及其对应画面动作。  
2. **`prompt_en` 必须由原文驱动**：把原文中的关键名词/动作译成可画元素（例：「水在他们的左右作了墙垣」→ vertical walls of sea water on left and right；「走干地」→ dry seabed path；「车辆马兵追赶」→ distant chariot silhouettes behind）。**不得**用与原文无关的科幻隧道、现代徒步等替换。  
...
5. **画面禁止文字**（标题与「释义说明，仅供参考」一律后处理叠字）。  
6. **禁止**：God anthropomorphic, faces close-up, blood；会幕≠蒙古包；过红海≠水顶隧道/岩壁峡谷。  
7. 输出字段：
...
```json
{
  ...
  "refs": ["出埃及记 14:21-29"],
  "verse_anchors": [
    {"quote": "水在他们的左右作了墙垣", "visual": "vertical sea-water walls L/R"}
  ],
  "inferences": ["ancient Near Eastern robes — not in verse, costume only"],
  "prompt_en": "……（200–360 words；开篇点明由哪些经文动作构成）",
  "negative": "water ceiling tunnel, canyon cliffs, modern clothes, text, faces, blood…",
  "checklist": ["open sky", "water walls", "dry ground", "procession", "distant chariots"],
  "overlays": [{"label": "海水墙垣", "hint": "left_water"}],
  "risk_notes": "拦截时保留形制动词，弱化专名"
}
```

##### 智谱侧拼装

最终调用：`model=cogview-3-flash`（除非显式指定备选），prompt ≈：

`{prompt_en}` + 固定尾句：「静穆纸感扁平插画；画面不要任何文字、字母、水印、标题条。」  

`negative` 并入 `Avoid:`。敏感拦截（1301）时按 `risk_notes` 软化专名、**保留原文驱动的形制动词**，再重试一次。

##### 质量门（入库前人工或半自动）

| 检查 | 通过标准 |
|------|----------|
| 形制 | 白名单要素 ≥ checklist 半数可辨 |
| 气质 | 纸感/低饱和；非电影海报 |
| 文字 | 画面无乱码汉字；标题仅叠字层 |
| 边界 | 无神人同形、无血腥近景 |
...
| 直接短句喂生图模型 | DeepSeek→智谱（默认 Flash） |
|--------------------|------------------|
| 模型补全成通用场景 | 强制经文 must_see / must_not |
| 中文形制词易被忽略 | 英文空间描述 + 材料名词更稳 |
| 标题乱码 | 标题移出模型 |

#### 19.14.7 讲解设计 · 如何把经文「讲懂」（2026-09-16）

> **纠正：** 视觉卡**不**以「一屏一事」限制信息密度；也**不**要求「一张生图塞进全部站点」。  
> **定案（修订）：** 「总图 + 分图」是**常用讲解模式**，**不是硬性套餐**。按场景选用合适构图即可：单卡器物、单站叙事、路径总览、总图+若干分图都可以。故事仍主要画在图里，文案是轻锚点。  
> **目标：** 先设计**怎么讲**，再决定**怎么画**；帮人建立脉络，不替代读经，不讲道，不 guilt。

##### 输入 → 讲解 → 画面

```
经文 Sources（refs + scripture_text）
  → 讲解设计（要讲清什么？需要几张图？）
  → 按场景选型生图（见下表；可 1 张，也可总图+分图）
  → 各图轻叠字（§19.14.10）；happen/link/quote 进点开层
```
...
| 图型 | 何时用 | 画面要求 | 生图建议 |
|------|--------|----------|----------|
| **单卡** | 器物、单场景、单站就够讲清 | 一个清晰 motif | 1 次 |
| **总图** | 行程/关系/时间线需要「地图感」 | 弧、顺序、地理关系；细节可省略 | 通常 1 次 |
| **分图** | 某站值得展开细看 | 该站核心 motif 清晰 | 按需，关键站才出 |
| **点开文案** | 溯源与串联 | `happen` · `link` · 原文 · 去读经 | **不**生图 |

**选用原则：** 能一张讲清就不要硬拆；行程长、站差异大再用总图+分图；次要站可以只有文案。
...
┌ 总图（可点热区 ①…⑦）              ┐  ← 进入专题先看总览
├ 站条 / 热区 → 分图半屏或全屏       ┤  ← 详细说明
│  分图 + 轻锚 + happen/link/原文    │
├ 上/下一站 · 回总图                 ┤
└ 相关专题 · 问小爱                   ┘
```

- 半屏列表 / 封面：**只加载总图**；分图 **点开再拉**（符合 §19.14.4 屏上要少）。  
- 分享：默认可分享当前层（总图或某一分图），不强制拼成长卷。

##### 生图调用规则（软约束）

| 规则 | 说明 |
|------|------|
| **按场景出合适的图** | 单卡 / 总图 / 分图自由组合；**不**强制每条专题都做满总图+全站分图 |
| **建议避免** | 把 N 张图 **横向拼接成一张** 交付（风格易漂）；用说明文字墙代替画面讲故事 |
| **DeepSeek 职责** | 按选题输出需要的 `prompt_en`（1 条或多条）+ `happen/link`；缺省站可不写分图 prompt |
| **叠字** | 轻锚 + 免责，见 §19.14.10 |
| **成本闸** | 分图只给关键站；列表/半屏优先加载封面那一张；其余点开再拉 |

> **与旧规关系：** 废止「必须单次塞全站」与「必须总图+全部分图」两种硬套餐。保留「故事画在图里」；构图形式服从讲解需要。

##### 讲解四问（写分镜前必答）

| 问 | 作用 | 旷野行程例 |
|----|------|------------|
| **这站看见什么？** | 决定分图母题 | 利非订 → 裂石涌水 |
| **经文说发生了什么？** | 一句事实，可溯源 | 「击打磐石，水就流出来」 |
| **它在整段弧里干什么？** | 写入总图顺序与 link | 旷野「供应与试炼」中的一环 |
| **读者最容易卡什么？** | 专名/地理/因果轻释 | 「玛拉」=苦；过海后仍会缺水 |

**禁止当作主讲解：** 人生应用三点、道德催促、与经文无关的百科堆砌。

##### 叙事弧（先弧后站）

长叙事先收成 **3～5 段弧**，站点挂在弧下；**总图表现弧与顺序**，分图表现站内细节。

**旷野行程（出 12–19）弧：**

| 弧 | 站点 | 读者应带走的理解 |
|----|------|------------------|
| **离开** | ①兰塞·疏割 | 奴役之地被抛在身后，行程开始 |
| **拯救** | ②红海 | 神开路；左右水墙、走干地 |
| **供应与试炼** | ③玛拉 → ⑥利非订 | 苦水、棕泉、吗哪、磐石——路上仍被养活，也仍会困乏 |
| **朝见预备** | ⑦西奈山 | 行程收束到「山下安营」，为立约/会幕作结 |

> 读者应能：**看总图回答「这段路整体在干什么」**；**看分图回答「这一站发生了什么」**。

##### 单站「讲解配方」（标准 4 字段）

| 字段 | 长度 | 用途 | 印在总图？ | 印在分图？ |
|------|------|------|------------|------------|
| `motif` | 可画物件名 | 分图生图约束；总图可用简化符号 | 符号级即可 | **画清楚** |
| `happen` | ≤18 字 | 经文事实白话 | 否（点开） | 可角标 1 行 |
| `link` | ≤16 字 | 与弧/上站关系 | 否 | 默认点开 |
| `gloss` | ≤10 字，可选 | 专名轻释 | 否 | 极少用 |

示例（⑥利非订）：

- `motif`: 裂开的磐石 + 流出的水  
- `happen`: 摩西击打磐石，水流出来给百姓喝  
- `link`: 旷野里再次经历「缺水 → 被供应」  
- `gloss`: （无，或「利非订：安营之地」）
...
| 层级 | 内容 | 原则 |
|------|------|------|
| **总图** | 路径/关系/时间骨架 + ①…⑦ 轻锚 | 建立「地图感」；**1** 次生图 |
| **分图** | 单站 motif 清晰叙事 | 详细说明；**按站**生图，点开加载 |
| **轻锚** | 编号、短站名、出处、免责 | §19.14.10 |
| **点开文案** | `happen` + `link` + 原文短引 | 溯源；**不**再生图 |
| **勿做** | 空总图 + 文字说明书墙；N 张横拼成一张「伪总图」 | 图被架空或风格失控 |

##### 旷野行程 · 完整讲解脚本（定稿示例）

| 序 | 站 | motif（分图画什么） | happen（讲什么） | link（为何接在这里） |
|----|----|---------------------|------------------|----------------------|
| ① | 兰塞·疏割 | 长队离开泥砖城邑 | 以色列人从兰塞起行往疏割 | **离开**：行程开始 |
| ② | 红海 | 左右水墙、中间干地 | 水分开，百姓走干地过海 | **拯救**：神开路 |
| ③ | 玛拉 | 泉旁、枯枝触水 | 水苦不能喝，后来可饮 | **试炼**：刚得救仍遇苦水 |
| ④ | 以琳 | 棕树与多股泉源 | 十二泉、七十棕树，在此安营 | **供应**：苦后有歇息之地 |
| ⑤ | 汛的旷野 | 地面白霜状小圆物 | 吗哪显明，日日可拾 | **供应**：路上的粮食 |
| ⑥ | 利非订 | 磐石裂开流水 | 击打磐石，水流出 | **试炼+供应**：再遇缺水 |
| ⑦ | 西奈山 | 山下帐篷环山 | 来到西奈旷野，在山下安营 | **朝见**：为听律/立约收束 |

**总图职责：** 示意「埃及一带 → 红海 → 旷野站点 → 西奈」的顺序与弧，站点用编号/简化符号即可。  
**总述一句（点开导语，不印满图）：**  
「出埃及记用一条路，把人从离开、拯救，带到旷野里的供应与试炼，最终停在西奈山下。」

##### DeepSeek「讲解设计师」输出（总图 + 分图）

输出 **1 条总图 prompt** + **每站可选分图 prompt**（有分图的站才写 `detail.prompt_en`）：
...
```json
{
  ...
  "arc": [
    {"name": "离开", "stops": [1]},
    {"name": "拯救", "stops": [2]},
    {"name": "供应与试炼", "stops": [3,4,5,6]},
    {"name": "朝见预备", "stops": [7]}
  ],
  "guide_one_liner": "……",
  "overview": {
    "prompt_en": "……（总图：路径/弧总览；站点符号级；画面无字；200–320词）",
    "negative": "photoreal collage, text, letters…"
  },
  "beats": [
    {
      "i": 1,
      ...
      "ref": "出 12:37",
      "detail": {
        "prompt_en": "……（分图：本站 motif 清晰；无字；180–300词）",
        "negative": "…"
      }
    }
  ]
...
1. **总图**调用 1 次 `cogview-3-flash`（`overview.prompt_en`）。  
2. **分图**对需要详细说明的站各调用 1 次（`beats[i].detail.prompt_en`）；无 `detail` 的站仅文案。  
3. 各图按 §19.14.10 轻叠字；`happen` / `link` / `quote` 进点开层。  
4. **禁止**把分图拼回一张长卷再当总图入库。

若总图盲测不清：减总图站点数/改用更简路径示意，细节下放到分图。  
若某分图不清：只重跑该站 prompt，不影响总图。

##### 质量门（讲解向）

| 检查 | 通过 |
|------|------|
| 结构 | 有总图；关键站有分图或明确「仅文案」 |
| 调用 | 总图 1 次；分图按站计数；**无** N 拼 1 |
| 总图盲测 | 遮字仍能答「整体在干什么 / 大致顺序」 |
| 分图盲测 | 遮字仍能认出该站核心 motif |
| 溯源 | 每站 `happen` 能指回原文 |
| 气质 | 无助人愧疚、不催进度 |
| 性能 | 首屏只解总图；分图懒加载 |

#### 19.14.8 知识专题 → AI 叙事主图（2026-09-16 认可）

> **范围：** 搜索内 **知识专题**（地图 / 关系 / 时间线 / diagram），**不含**已删除的人生主题。  
> **原则：** 经文仍是本体；图帮读懂脉络；按场景选用合适图（§19.14.7，非硬套餐）；先写讲解再画；避免「空图 + 大段后叠卡」当主体验。

| 专题类型 | 现状 | 适合的图 | 调整方式 |
|----------|------|----------|----------|
| **行程 / 地理**（旷野、保罗宣教、加利利事工） | 站名 + note | **总图**路径/弧 + **分图**各站 | 封面=总图；点站出分图；站列表缩成轻锚 |
| **关系专题**（出埃及核心人物、门徒、同工等） | 人物 + 长 note | **总图**关系示意 + **分图**关键事件母题 | note 进点开；重要 beat 可有分图 |
| **器物 / 圣所 / 礼仪**（会幕、约柜、逾越节门楣等） | 说明文 / SVG | 总览（外观/区位）+ 构件分图；精确尺寸仍优先 SVG | 一物可一总+多分；短 happen |
| **时间线**（犹大诸王、耶稣生平） | 年代 + note | **总图**轴骨架 + **分图**节点母题 | 详注点开；节点按需出分图 |
...
┌ 总图（AI）· 轻锚 ①…⑦ / 弧名       ┐  ← 主视觉总览
├ 点热区或站条 → 分图（详细说明）    ┤
│  分图 + happen · link · 原文 · 读经 │
├ 上/下一站 · 回总图                 ┤
└ 相关专题 · 问小爱                   ┘
```

#### 19.14.9 适合图片展示说明的圣经内容清单（扩库优先级）

> **筛选标准：** 有可画的空间/器物/路径/礼仪形制；能帮「读懂」；符合 §19.14.1–2 白/黑名单。  
> **不做：** 抽象教义图、神人同形、血腥近景、人脸崇拜像、人生处境鸡汤海报、词典每节配图。

##### A. 已有资产（巩固为叙事主图 / 示意卡）

| 类 | 现有 id / 样张 | 视觉动作 |
|----|----------------|----------|
| 地图 | `exodus-wilderness` · `paul-first-journey` · `jesus-ministry-galilee` | 行程 → 一路多景或路径叙事主图 |
| 时间线 | `kings-of-judah` · `life-of-jesus` | 节点母题微缩轴 |
| 关系 | `exodus-core` · `twelve-disciples` · `david-line` · `paul-companions` · `patriarchs` · `jesus-ministry-places` | 关系叙事主图（符号级，无写实面容） |
| Diagram SVG | 会幕平面 · 约柜 · 圣殿分区 · 逾越节门楣 · 过红海 · 陈设饼桌 | **精确布局保留 SVG**；氛围外观可另做位图卡 |
| 样张 | 会幕外观 · 圣殿远景 · 过红海 · 保罗路径 · 旷野行程故事卷 | 入库压 WebP 双档 |

##### B. P0 扩库（器物 / 圣所 · 读经高频）

| 题材 | 建议画面 | 锚点经文（例） |
|------|----------|----------------|
| 金灯台 / 七灯 | 静物示意 | 出 25:31–40 |
| 香坛 · 铜祭坛 · 洗濯盆 | 单物件或会幕内区位 | 出 27–30 |
| 大祭司胸牌 / 以弗得 | 器物特写（无人脸） | 出 28 |
| 至圣所幔子 · 裂开示意 | 空间+幔，远景 | 出 26；太 27:51 |
| 所罗门圣殿外观 / 院落 | 远景建筑 | 王上 6–7 |
| 约柜抬行过约旦 | 河水分开 + 祭司抬柜远景 | 书 3–4 |

##### C. P1 扩库（行程 / 地理 · 一条弧一张图）

| 题材 | 建议画面 | 锚点（例） |
|------|----------|------------|
| 亚伯拉罕行程 | 吾珥→哈兰→迦南路径微景 | 创 11–12 |
| 约瑟下埃及 | 路途/井/法老仓示意（无近脸） | 创 37–41 |
| 征服迦南要点 | 耶利哥城墙坍塌远景等少量站 | 书 6 等 |
| 被掳与归回 | 耶路撒冷→巴比伦 / 归回路线 | 王下 25；拉 1 |
| 保罗第二 / 三次宣教 | 同第一次路径图结构 | 徒 15–21 |
| 保罗赴罗马 | 海路风暴→岸示意 | 徒 27–28 |
| 受难周耶路撒冷路线 | 城内关键地点轻路径 | 四福音末段 |

##### D. P1 扩库（叙事场景 · 空景 / 母题，非群像写实）

| 题材 | motif 方向 | 注意 |
|------|------------|------|
| 创造七日母题条 | 光 / 水陆 / 星 / 植物 / 安息空景 | 符号化，勿科幻地球仪 |
| 方舟与洪水 | 远景方舟+水线 | 无血腥动物特写 |
| 巴别塔 | 远景塔与分散路 | 非现代摩天楼 |
| 燃烧的荆棘 | 火焰荆棘空景 | 无神形体 |
| 西奈颁布 | 山+云+山下帐 | 无写「十诫」汉字在像素里 |
| 空坟墓 / 客西马尼园 | 空园、空墓门口 | 无耶稣写实面容 |
| 五旬节上层楼 | 空室+风/火焰符号 | 禁止火舌扑脸特写 |
...
|------|------|------------|
| 住棚节棚子 | 枝叶棚示意 | 利 23 |
| 赎罪日礼仪区位 | 大祭司进至圣所示意（背影/无脸） | 利 16 |
| 五旬节初熟 | 麦捆/饼 | 利 23；徒 2 背景可分卡 |
| 吹角节 | 羊角号静物 | 利 23 |

##### F. 明确不做（再强调）

- 人生主题意象海报（盼望锚、焦虑卸担等）作为产品专题  
- 恩典 / 信心 / 三位一体等抽象教义几何图  
- 耶稣 / 先知 / 使徒写实肖像  
- 战争血腥、神迹「特效大片」脸部特写  
...
1. **先吃干现有专题**（§19.14.8：地图 3 + 关系 6 + 时间线 2 + diagram 氛围补洞）  
2. **再 P0 器物**（与读经半屏「见图」同库存）  
3. **P1 行程与空景叙事**按书卷懒加载云端  
4. 每张遵守：原文驱动 · 图型按场景选用 · 右下角免责 · WebP 双档 · 叠字按 §19.14.10  

#### 19.14.10 中文叠字后处理 · 提升方案（2026-09-16）

> **问题：** 早期样张用「粗白底横条 + 黑体硬贴」，易显廉价、挡母题、半屏缩略时糊成一片。  
> **定案：** 生图仍 **零文字**；叠字走 **统一排版引擎**（建库脚本 + 可选端上运行时层）。目标：**可读、静穆、不抢故事、可复现**。

##### 原则

| # | 原则 | 说明 |
|---|------|------|
| 1 | **字不进模型** | 标题 / 站名 / 出处 / 免责只走后处理；禁止靠 prompt「写清楚汉字」 |
| 2 | **字不抢故事** | 叠字总占地 **≤ 画面面积约 12%**；讲解正文进点开层，不印满主图 |
| 3 | **对比自适应** | 按落点区域明暗选字色与底衬；禁止一律不透明大白条 |
| 4 | **双通道交付** | **分享/导出图**可烘焙叠字；**App 内半屏/全屏**优先运行时叠字（跟 DPR、无障碍字号） |
| 5 | **安全区预留** | 生图 prompt 要求底边 / 指定角 **留低细节带**，给标题与免责腾位置 |

##### 信息层级（从上到下优先级）

| 层级 | 内容 | 默认位置 | 字号（相对 full 1440 宽） |
|------|------|----------|---------------------------|
| **L1 标题** | `title_zh`（≤8 字为宜） | 左下或左上 **安全区内** | ~42–52 px；字重 Medium/Semibold |
| **L2 出处** | `refs` 一行（如「出埃及记 14:21-29」） | 紧贴标题下方 | ~22–26 px；字重 Regular；略降不透明度 |
| **L3 轻锚** | ①…⑦ 短站名 / 部位标签（≤4 字） | 靠 motif，**避开画面视觉中心** | ~18–22 px |
| **L4 免责** | 固定「释义说明，仅供参考」 | **右下角**，距边 ≥2.5% | ~14–16 px；不抢 L1 |

禁止：居中通栏大标题、居中底栏免责墙、L3 超过 8 个标签、把 `happen/link` 长句印上主图。

##### 可读性技术（比白底条更好）

按场景选用，**默认可组合「浅 scrim + 描边」**：

| 手法 | 何时用 | 规格要点 |
|------|--------|----------|
| **区域采样自适应** | 默认 | 在文字 bbox 扩 8–12px 采样平均亮度；亮底 → 深字 `#2C2825`；暗底 → 浅字 `#F7F3EC` |
| **柔和 scrim（推荐）** | 标题 / 出处 | 半透明圆角条：`rgba(247,243,236,0.72)` 或暗版 `rgba(44,40,37,0.55)`；**高度贴内容**，左右 padding ≥ 字号 ×0.45；圆角 ~字号 ×0.35；**禁止**通栏粗白砖 |
| **双描边 / 晕影** | 轻锚编号、免责 | 字色 + 1.5–2px 反色描边，或极轻 drop-shadow；**无底板**也可读 |
| **毛玻璃（可选）** | 分享图精品档 | 对底板区域轻度 blur + 半透明填色；建库成本略高，半屏列表勿用 |
| **废止** | — | 不透明纯白通栏、强黑描边卡通字、系统默认无 CJK 回退方框 |

**字体：** 建库存 **思源黑体 / Noto Sans SC**（或 PingFang 仅 macOS 本地预览）；打包进叠字工具，**禁止**依赖机器未装字体导致方框。字距略松（tracking +2%～4%），避免「挤成墨团」。

##### 落点与避让

```
生图（无字）→ 可选：安全区掩膜 / 显著度粗检
  → 放置 L1/L2（优先预留角）
  → 放置 L3（hint → 坐标；碰撞则外推或降级隐藏）
  → 放置 L4（右下固定，与 L1 对角，避免同角互撞）
  → 对比度抽检 → 压 WebP
```

| 规则 | 说明 |
|------|------|
| **安全边距** | 四边 inset ≥ 画面短边的 **3.5%**；刘海/圆角机型另加 UI safe（运行时） |
| **hint 坐标** | DeepSeek `overlays[].hint`（如 `left_water`）映射到相对坐标表；人工可覆写 `x,y`（0–1） |
| **碰撞** | L3 与 L1/L4 bbox IoU > 0.05 → 外推；仍撞则 **丢弃该 L3**（故事靠图，不靠字） |
| **母题保护** | 画面中心 40%×40% 默认禁放大标题；L3 可点状进入但字要短 |
| **prompt 配合** | 固定尾句加：「leave a quiet low-detail band along the bottom-left for a short caption; no text in pixels」 |

##### 烘焙 vs 运行时

| 通道 | 用途 | 叠法 |
|------|------|------|
| **烘焙进 WebP** | 分享卡、运营导出、无客户端渲染的静态页 | 建库脚本一次合成；元数据仍保留纯净 `clean` 原图 URL（可选） |
| **运行时叠字（推荐 App 内）** | 读经半屏 / 全屏细看 | 下发 **无字 clean 图** + `overlay_spec` JSON；Flutter/Web 用同一 token 绘字 → 跟系统字号、暗色模式、无障碍 |
| **混合** | thumb 可轻烘焙标题以便列表一眼认；full 用运行时 | thumb 只 L1（甚至只标题），免责可省略到点开再显 |
...
```json
{
  ...
  "anchors": [{"label": "海水墙垣", "x": 0.18, "y": 0.42}],
...
| 检查 | 通过 |
|------|------|
| 对比 | 文字与局部底对比大致 ≥ 可读（抽样失败则换 scrim / 换字色重渲） |
| 覆盖 | 叠字像素占比 ≤ ~12%；L3 ≤ 8 个 |
| 字体 | 无 tofu（□□）；CJK 字形完整 |
| 撞车 | L1 与 L4 不同角；无标签压在关键 motif 正中 |
| 气质 | 无海报大字报、无荧光色、无模型名水印 |
| 双档 | thumb / full 各渲一版或运行时按档缩放字号（字号 ∝ 宽度） |

##### 工程落地（建议）

| 步 | 做法 |
|----|------|
| 1 | 抽公共 `visual_card_overlay`（Python 建库 + 与 Flutter/Web token 对齐的字号比例表） |
| 2 | 字体子集（仅常用标题用字可再缩包；免责固定句可预渲染小 PNG 亦可） |
| 3 | CI：对样张跑「无字原图 + spec → 叠字」金样对比（允许抗锯齿差，禁位置漂） |
| 4 | 旧样张：不强制回刷；**新图**一律走本规格 |

##### 一句话

**模型只出无字故事图；叠字用自适应对比 + 轻 scrim + 层级落点；App 内能运行时叠就运行时叠，分享再烘焙。**

#### 19.14.11 可读性方案 · 「讲得清」优先（2026-09-16 定稿）

> **问题（样张复盘）：** 现有生图易成「大面积留白 + 气氛剪影 + 仅标题出处」——半屏缩小时**看不清在讲什么**；总图航线过淡、分图母题过少。  
> **方向：** 不回到说明书文字墙；改为 **信息架构先定 → 画面必含可读母题 → 叠字承担顺序与短事实**。  
> **状态：** **已拍板并出样张**（`exodus_wilderness_v311` / `paul_first_journey_v311`）。

##### 诊断（为何「信息太少」）

| 现状 | 后果 |
|------|------|
| Prompt 偏「静穆留白 / 远景剪影」 | 缩略图只剩色块 |
| 叠字只有标题+出处 | 读者不知道「发生了什么」 |
| 总图路线细、站点无编号 | 看不出顺序与弧 |
| 分图 1 个氛围场景撑满 1440×720 | 缺少可指认的物件线索 |

##### 新原则（取代「越空越高级」）

1. **半屏可读：** 缩到手机半屏宽，仍能答「这张在讲哪一件事」。  
2. **母题 ≥3：** 每张叙事/分图至少 **3 个可指认视觉线索**（物件或空间关系），禁止「只有人和空房间」。  
3. **字图分工：** 图负责「看见什么」；叠字负责「顺序 + 一句事实」；长解释仍进点开。  
4. **图型服从讲解：** 能一张讲清用单卡；讲不清就加分图或改成「路径图+图例」，不硬留白。

##### 三种推荐版式（按场景选，非硬套餐）

| 版式 | 适用 | 画面里必须有 | 叠字必须有 |
|------|------|--------------|------------|
| **A · 路径解说图** | 行程总览（保罗、旷野） | 粗可辨航线、站点圆点、海陆色差；可含 1 条回程示意 | ①…⑦ 站名芯片 + 底栏标题/出处；可选极简图例「去程/回程」 |
| **B · 单站说明图** | 分图 / 单卡叙事 | **前景清晰母题**（占画面 ≥35% 注意力）+ 2 个背景线索；少大面积空天空 | 站名 + **一行 happen（≤14 字）** + 出处；可选 1–2 个部位标签 |
| **C · 对照/结构图** | 器物、会幕区位、关系 | 分区/零件/连线清楚（可偏示意，接近 diagram） | 零件名标签；精确尺寸仍可 SVG |

**明确少用：** 大面积负空间海报风、电影级剪影群像、只有情绪没有物件的「氛围卡」。

##### 信息密度目标（可验收）

| 检查 | 通过标准 |
|------|----------|
| **3 秒测试** | 遮掉点开文案，只看图+叠字，能说出本张主题 |
| **母题** | 叙事卡可数出 ≥3 个线索（例：路司得=柱廊 + 举手人群剪影 + 拄拐/伸手姿态） |
| **总图顺序** | 不看说明也能指出大致出发→折返 |
| **叠字占比** | 标题+happen+站名合计大约 **8%–18%** 画面（可高于旧「≤12%」上限）；仍禁止大段 bullet 墙 |
| **happen** | 分图默认叠 **1 行事实**（来自讲解配方）；不是可选装饰 |

##### 生产流程（仍 DeepSeek → 生图 → 叠字）

```
讲解卡（arc / motif×3 / happen / link）
  → 选定版式 A/B/C
  → DeepSeek：prompt_en 必须列出 must_see 清单（≥3）并写入构图位置
  → 生图（无字）
  → 叠字引擎：标题 + happen + 编号站名/部位（§19.14.10 升级）
  → 3 秒测试；不通过则改 prompt 或加叠字，不靠再铺说明卡
```
...
```json
{
  "layout": "A_path | B_beat | C_structure",
  "must_see": ["线索1", "线索2", "线索3"],
  "happen_overlay": "≤14字事实句",
  "stop_chips": [{"i": 1, "label": "安提阿", "x": 0.82, "y": 0.48}]
}
```

##### 以「保罗第一次宣教」为例（重做规格 · 暂不出图）

| 图 | 版式 | 必须让人看懂 |
|----|------|----------------|
| 总图 | A | 安提阿→塞浦路斯→小亚细亚内陆→折返；叠 ①–⑦ 站名 |
| 差遣 | B | 按手/祷告圈 + 室内窗光 + 两人被围绕；叠「圣灵分派，按手差遣」 |
| 塞浦路斯 | B | 海船 + 岛屿岸线 + 会堂/柱廊；叠「坐船往塞浦路斯传道」 |
| 彼西底安提阿 | B | 会堂讲台区 + 听者背影排 + 卷册/灯；叠「安息日在会堂讲道」 |
| 路司得 | B | 柱廊外邦氛围 + 得医治姿态线索 + 人群举手（误会）；叠「医治后被人当作神」 |

##### 与旧样张关系

- 旧空旷样张 / v311 底栏重叠字样张可留作对照。  
- **v311 结论（复盘）：** 加厚底栏叠字 → **字挡画面**，信息仍不够；不能靠「再多叠一点字」解决。  
- **下一版规格见 §19.14.12（画面与讲解分离）。**

#### 19.14.12 讲解页结构 · 告别「一张大海报」（2026-09-16 提案）

> **用户反馈：** 仍是一张大图为主；文字遮挡画面；能给出的信息少。  
> **判断：** 瓶颈不在模型「画得不够美」，而在 **把讲解塞进同一张位图**。  
> **定案方向：** **图只负责看见；讲解在图外的 UI 层**；需要多信息时用 **多格 / 列表 / diagram**，不在像素里堆底栏。  
> **状态：** **已拍板（2026-09-16）**；半屏原型见 `data/visual_cards/samples/knowledge_page_prototype/`。

##### 为什么 v311 仍不够

| 做法 | 结果 |
|------|------|
| 一张 1440×720 叙事海报 | 半屏里只有一个气氛场景 |
| 底栏叠标题+happen+芯片（占 ~25% 高） | **挡住画面**；字一多更挤 |
| 总图靠 AI 写站名 | 乱码字母；中文只能再叠一层 |

##### 新结构（推荐默认）

```
┌─────────────────────────────┐
│  干净图区（无中文底栏）        │  ← AI / SVG 只出无字图
│  可含极轻编号圆点（可选）      │
├─────────────────────────────┤
│  讲解条（App UI，不烤进图）    │  ← 标题·出处·happen·站列表
│  可滚动：①…⑦ / link / 原文   │
└─────────────────────────────┘
```
...
| 层 | 内容 | 规则 |
|----|------|------|
| **图区** | 无字（或仅无字编号圆点） | **禁止**大底栏、禁止烤中文 happen |
| **讲解条** | 标题、出处、happen、chips、上/下一站 | 系统字体；不挡图；可滚动 |
| **点开** | link、原文、问小爱、去读经 | 半屏已有空间就展开，不必再叠像素 |

分享导出：用 **「上图下文」合成一张长图**（图完整 + 下方 UI 截成面板），**不是**把字盖在画面上。

##### 三种信息密度策略（按场景选）

| 策略 | 何时 | 怎么做 | 信息量 |
|------|------|--------|--------|
| **1 · 图文分栏** | 单站叙事 | 上图（干净）+ 下讲解条 | 中：一句 happen + 2–3 chip |
| **2 · 多格叙事** | 一站有多个动作 / 总览想一眼多景 | **2×2 或 1×3 格**（一格一事）；每格小图+格下短标 | 高：一屏 3–4 个母题 |
| **3 · Diagram 主路径** | 行程/区位 | **SVG/矢量路径图**挂站名（可点）；站旁可挂 **小缩略 AI  vignette** | 高：顺序+地名+可点分图 |

**默认建议：**

- 保罗 / 旷野 **总览 → 策略 3（diagram 路径）**，不要再指望 AI 大海报地图讲清站名。  
- 单站 **→ 策略 1**；若一站情节多 **→ 策略 2**。  
- 器物精确布局 **→ 继续 diagram SVG**（§19.14 既有）。
...
┌─────┬─────┐
│ 格1  │ 格2  │   每格：一个清晰母题（无字）
├─────┼─────┤   格下 UI 短标：①名 · happen≤10字
│ 格3  │ 格4  │
└─────┴─────┘
```

- 生图：可 **一次 prompt 要求四格分镜**（带 gutter），或 **四次小图 UI 拼**（拼的是页面布局，不是横贴成长海报冒充总图）。  
- 验收：遮掉长文，仍能说出 3–4 个情节点。

##### 路径总览（策略 3）规格摘要

| 元素 | 实现 |
|------|------|
| 底图 / 航线 / 站名 | **手绘或 SVG diagram**（中文站名矢量字，清晰可点） |
| 站热区 | 点开 → 策略 1 分图+讲解条 |
| AI 角色 | 可选：站的 **小 vignette 缩略**，不替代主路径图 |

##### 对 §19.14.10 / v311 的修正

| 旧 | 新 |
|----|-----|
| 分享图默认烤大底栏中文 | **默认不挡图**；分享用上图下文合成 |
| 靠叠字提升信息量 | 靠 **UI 讲解条 + 多格/diagram** |
| 行程总图以 AI 位图为主 | 行程总图以 **diagram 为主** |

##### 落地顺序（建议）

1. ~~半屏 HTML 原型~~ ✅ `data/visual_cards/samples/knowledge_page_prototype/index.html`  
2. 旷野/保罗 **总览改生产级 SVG 路径图**（线框 → 精绘）  
3. 分图资产改为 **无烤字 clean 图**；讲解只走 UI；再按需补多格母题生图  

#### 19.14.13 NotebookLM 式「知识信息图」逻辑 · 彼爱怎么对齐（2026-09-16 定稿）

> **问题：** NotebookLM Infographic / Slide Deck 看起来「一张图信息很多」；我们用 CogView 单海报却又空又挡字。  
> **结论：** 要对齐的是它的 **流水线逻辑**，不是去换一个更会画大海报的模型。  
> **状态：** **已拍板**；与 §19.14.12 一并落地原型。

##### NotebookLM 在做什么（逻辑拆解）

公开能力大致是：**Sources（你上传的材料）→ 结构化提炼 → Nano Banana Pro 等图像模型按「信息图/幻灯」版式出图**（可选横竖、简洁/标准/详尽、自定义风格提示）。要点不是「画一张风景」，而是：

| 步骤 | NotebookLM | 彼爱对应 |
|------|-------------|----------|
| **1. 锚定来源** | 只吃 notebook 里勾选的 sources | **经文原文 + 讲解配方**（`scripture_text` / refs），禁止凭印象 |
| **2. 先做知识结构** | 研究/大纲 agent：论点、关系、顺序、隐喻 | 已有：**叙事弧 + motif/happen/link**（§19.14.7） |
| **3. 再选「信息图版式」** | Infographic / Slide Deck（多区块、标题、步骤、关系） | **不要默认单场景海报**；用 **多格 / 路径 diagram / 上图下文页** |
| **4. 视觉引擎服从版式** | 图像模型被提示去填「带标题区、分区、图标」的版面 | 中文 **不信任生图写字** → **UI/SVG 出字**；AI 只填无字母题格 |
| **5. 密度可调** | concise / standard / detailed | 产品档：半屏摘要 / 点开详解 / 分享长图 |

它「一张里信息多」的本质往往是：**版面上同时出现多个信息区块**（标题带、步骤条、小图、关系线），而不是一块留白风景。

##### 彼爱不要照搬的两点

| NotebookLM 习惯 | 彼爱边界 |
|-----------------|----------|
| 图内大量英文/标题由模型直接写 | 中文易乱码 → **字在 UI/SVG**（§19.14.10/12） |
| 通用研究笔记→信息图 | 我们是 **读经辅助**：静穆、可溯源、不 guilt、不替代经文 |
| 输出常是整页位图（难二次编辑） | 优先 **可点组件**（站、格、原文），位图只作母题素材 |

##### 彼爱达成「那种效果」的产品公式

```
经文 Sources
  → 讲解设计师（弧 / 站 / happen / must_see）     ← 对齐 NotebookLM 的结构层
  → 版式选择：
       · 行程总览 = SVG diagram（站名可点）
       · 单题详解 = 多格信息页（2×2 / 步骤条）
       · 单站深看 = 干净母题图 + UI 讲解条
  → AI 只生成「格内无字母题」或小 vignette
  → 客户端拼装成「知识页」（字永不挡图）
```

**一句话：** NotebookLM = **结构化知识 × 信息图版式 × 生图填格**；彼爱 = **经文结构 ×（diagram + 多格 UI）× CogView 只填格**。
...
1. **先有骨架**（路径或步骤，一眼顺序）  
2. **每格/每站有一句事实**（happen，在图外）  
3. **点开有原文与去读经**（溯源，对齐 sources）  
4. **画面是母题插图，不是整页唯一信息载体**

##### 与 §19.14.12 的关系

§19.14.12 解决「字挡图」；本节说明 **为何要对齐 NotebookLM 的「结构→版式→填格」**，而不是继续优化单张大海报 prompt。

#### 19.14.14 「结构→版式」流水线 · 落地规格（2026-09-16）

> **目标：** 把 §19.14.13 的公式变成可编译、可 API、可 UI 消费的产物，而不是散落在前端硬编码。  
> **状态：** **已接通**（行程类三专题）；有 layout 的专题默认 **Explainer 一页看懂**；逐站导览为次要模式。

##### 产物

| 项 | 规格 |
|----|------|
| Schema | `knowledge_layout@1`（`data/knowledge/layouts/schema.json`） |
| 编译器 | `scripts/compile_knowledge_layout.py`：读 `map_tours` + 策展补丁 → 写出 layout JSON |
| 产出目录 | `data/knowledge/layouts/{id}.json` + `index.json` |
| API | `GET /content/knowledge-layouts` · `GET …/{id}` · **`POST …/from-scripture`（原文→layout 预览，不落盘）** |
| Web 默认 | **`KnowledgeExplainerPage`**：竖向**故事脊**（编号 · vignette · happen · link · 出处）为主；示意路径默认折叠；底栏「分享信息图」 |
| Web 次要 | `MapStoryMode` 逐站导览：入口 `?mode=tour`（Explainer 内「改为逐站走」） |
| 分享 | **上图下文信息图**（`knowledge_infographic_share`）：干净母题 + **故事脊**下文（编号/happen/可选 link）；字不烤进图 |

##### 流水线（工程）

```
map_tours（结构源：站序 / ref / note / ask_seed）
  + CURATED 策展（arc / happen / link / chips / must_see / vignette）
  → compile_knowledge_layout.py
  → knowledge_layout@1
  → API → KnowledgeExplainerPage（默认）/ MapStoryMode（?mode=tour）
  → AI 填格（可选，仅 vignette；禁止烤中文底栏）
```
...
| ID | 用途 |
|----|------|
| `path_grid_explain` | 行程默认结构模板（编译产出）；UI 默认用 Explainer 故事脊消费 |
| `grid_explain` | 无路径示意时的多格页 |
| `explain_only` | 仅讲解条（轻量） |
| `diagram_hotspots` | 器物/礼仪 diagram 热点（后续） |

##### 纪律

1. **结构先于像素**：改讲解先改 `map_tours` / CURATED，再编译；不要在 UI 里再抄一份 happen。  
2. **字不进位图**：`fill.policy` 写明 AI 只填无字母题。  
3. **同 place 多站**：用 `beat_overrides_by_order`（如安提阿回程）。  
4. **缺策展时**：编译器从 `note` 启发式压一行 happen，保证版式仍可渲染。  
5. **默认 Explainer**：有 layout 时首屏讲清故事脊；路径图与逐站导览降为次要。
...
重编译：`python scripts/compile_knowledge_layout.py`

任意经文预览（不落盘）：`POST /content/knowledge-layouts/from-scripture`  
body: `{ title, refs, scripture_text, era_geo? }` → `{ layout, persisted:false }`

#### 19.14.15 主交付「密图」· 手机竖图 MVP（2026-09-17 拍板）

> **问题：** Explainer / 上图下文能讲清结构，但用户感知的「NotebookLM 信息图」是 **带走一张字很多的图**。  
> **拍板：**  
> 1. **主交付 = 密图**（单张竖构图，标题/弧/站序/happen/link/出处/免责 **全部在图内**）。  
> 2. **MVP 画幅 = 手机竖图**（建议 **1080×1920**，可分享到微信/朋友圈）。  
> **状态：** **保罗案例已出样**（`paul-first-journey` 密图 1080×1920）；通用任意经文入口待铺开。Explainer / `?mode=tour` / 上图下文保留为次要与过渡，**不再当作主路径。**

##### 用户感知

打开生成结果应直接看到 **一张可 pinch / 保存 / 分享的密图**；不是先读一页 UI 再点导出。可点读经、问小爱为次要动作（预览页底栏或长按）。

##### 工程公式（不变逻辑、换主输出）

```
经文 / 策展 layout（knowledge_layout@1）
  → 竖版信息图模板（journey_spine 等）
  → 版式引擎排字 + 矢量示意 + 可选 vignette 填格
  → PNG 1080×1920（字在图内；AI 不写字）
```
...
|----|------|
| 画幅 | **1080×1920** 竖图 |
| 首发题材 | **行程类**（复用保罗 / 旷野 layout；再接 `from-scripture`） |
| 入口 | 知识专题页主 CTA「生成信息图」；读经半屏入口 V1 |
| 合成 | 服务端模板渲染 → PNG（推荐 HTML/SVG→截图或 Satori/Resvg） |
| 填格 | 有 vignette 则贴入；无则色块+文字仍出完整密图 |
| 页脚 | 「释义说明，仅供参考」+ 彼爱品牌 |
| 明确不做（MVP） | AI 烤整页中文；A4/横版；关系/器物全模板库；我的信息图库 |

##### 保罗案例（已落地）

| 项 | 路径 |
|----|------|
| 密图 PNG（列表脊） | `apps/web/public/knowledge/infographics/paul-first-journey.png` |
| 密图 PNG（纸稿手绘） | `apps/web/public/knowledge/infographics/paul-first-journey-comic.png` |
| 样例目录 | `data/visual_cards/samples/paul-first-journey_comic_dense/` |
| 重渲 | `python3 scripts/render_comic_dense_infographic.py --id paul-first-journey` |
| 入口 | `/search/map/paul-first-journey`（优先 comic 密图） |

#### 19.14.16 密图视觉语言 · 手绘复古纸稿（2026-09-17 拍板）

> **方向：** 在 §19.14.15「密图主交付」之上，定视觉气质为 **手绘复古纸稿**；可使用 **原创动画形象**（禁止第三方动漫 IP）；**不同专题用不同绘制方案**，目标是打开就有「惊艳感」，而不是通用清单 UI。  
> **状态：** 规格拍板；保罗行程首发纸稿样例迭代中。

##### 气质三原则

1. **纸稿感**：泛黄纤维底、墨线边框；右上角统一圆形 **「彼爱手稿」** 印章（每张密图仅此一枚品牌标，禁「经文信息图」等文案条）；像一页被认真画过的讲经手稿，不是 App 卡片堆叠。  
2. **手绘角色**：每专题固定 1–2 个 **原创** 角色（如「小保罗 / 小巴拿巴」线稿或平涂动画风），用气泡/手势串叙事；**禁止**蜡笔小新等第三方形象。  
3. **专题分方案**：同一引擎、不同「墨色 + 道具母题 + 角色造型」；用户应能凭画风认出「这是旷野 / 这是保罗 / 这是会幕」。

##### 专题绘制方案（首批）

| 专题类型 | 墨色主调 | 角色 / 道具 | 版式母题 |
|----------|----------|-------------|----------|
| 宣教行程（保罗） | 地中海青绿 + 陶土褐 | 行者双人组、帆船、会堂柱 | 三弧面板 + 路径墨线 |
| 旷野行程（出埃及） | 沙金 + 夜蓝 | 摩西杖、云柱火柱、营寨 | 纵深路径 + 奇迹呼出 |
| 器物 / 会幕 | 圣所金 + 帷幔紫 | 祭司剪影、器物剖面 | 热点剖视图 |
| 人物关系 | 墨黑 + 朱红点 | 头像章、丝带连线 | 关系辐射网 |
| 教导要点 | 羊皮纸浅褐 + 靛蓝 | 书卷、灯台 | 论点条 + 经节脚注 |

##### 工程纪律（与 §19.14.12 对齐）

- **中文仍由版式引擎排字**；AI / 手绘位图只出角色与场景母题（可含极少量装饰符号，禁止大段中文烤进图）。  
- 填格 prompt 默认加：`hand-drawn ink on parchment, vintage manuscript, no photorealism, no modern UI`。  
- 每专题维护 `illustration_scheme`（墨色、角色资产目录、边框装饰、印章文案）。

##### 惊艳感检查（评审三问）

1. 遮掉标题，只看画风，能否猜出专题类型？  
2. 3 秒内能否抓住「一段故事弧」？  
3. 是否愿意转发朋友圈（纸稿完整、字可读、不廉价）？

#### 19.14.17 手稿册 · 总→站翻页（2026-09-17 拍板）

> **问题：** 总密图适合「一页看懂结构」，但单站信息不够细；若把细则塞回总图会糊、字碎。  
> **拍板：** 行程类密图升级为 **手稿册**：第 0 页 = **总手稿**（现有 comic 密图）；第 1…N 页 = **站手稿**（每站一张同规格竖图，大图 + 更细说明）。  
> **状态：** 交互与边界已拍板；保罗样张待出。
...
| 页 | 回答 | 内容重心 |
|----|------|----------|
| **总手稿** | 这段旅程怎么串起来？ | 弧、站序、每站一句 happen（现样合适，不加重） |
| **站手稿** | 停在这一站能多懂什么？ | 大 vignette + happen + link + 扩写 note + 可选 1–2 节短摘 |

##### 交互（小红书式）

- **专题页**：只展示 **封面卡片**（总手稿作封面图，首页轨卡气质）；点击进入全屏查看。  
- **全屏查看**：左右横滑翻页；支持双手捏合缩放；左上角 **返回**（关全屏回封面）；右上角 **系统分享**（当前页）。  
- **边界：** 末页再向后滑无效；**首页再向上一页方向滑（手指右滑）→ 退出专题**（回列表）。缩放中不翻页。  
- **不做**站序号条、「第 N 站」Chip、读经/问小爱 CTA。  
- **首页入口：** 今日推荐第 4 坑为 **探索**（**固定插图底** + **默认保罗首发**短名/导语；点进该专题全屏手稿，**打开落在第 1 页**；轻点图片切换沉浸顶栏；从首页进入后返回落在 `/knowledge`）；成长区去重主题卡。  
- **页码：** 打开全屏**始终从第 1 页**起；本机仍记进度，列表卡可显示「续 · N」作提示（不自动跳页）。  
- **弱网：** 全屏仅挂载当前页 ±1（邻页预取 WebP）；列表封面用轻量图，勿直接用 1080×1920 comic。手稿 PNG 旁提供同名 **WebP**（体积约 1/10）。  
- **安卓：** 探索手稿走 **H5 白名单**（与 iOS 同套），**不**再做 Flutter 原生手稿壳；`/search/map/:id` 手稿进叠层 H5。  
- **介质角标：** 列表支持「行程 / 笔记」弱标签；有音/视频时显示「听 / 看」；全屏底栏可播（有 `media` 时）。  
- **列表筛选：** 全部 / 行程 / 笔记。  
- **管理员新建：** `/knowledge/new`（需管理员）对齐小红书发笔记：先选 **图文 / 文字 / 音频 / 视频**；编辑态 **媒体大图优先**、顶栏取消/草稿/发布、底部册页条（＋加同类型页、···选类型、拖拽排序、点 × 删页，最多 12）；发布时文页烘焙为竖版纸页图；支持草稿与下架。  
- **明确不做：** 笔记**定时发布**（本期不做）。

##### 站手稿版式（同族）

- 画幅仍 **1080×1920**；右上角仅一枚圆形 **「彼爱手稿」**；纸稿气质与总图一致（§19.14.16）。  
- 布局建议：眉（站序/地名/出处 + 迷你路径高亮当前站）→ 大图约半屏 → 文三块（发生 / 脉络 / 多懂一点）→ 页脚免责。  
- 数据优先复用 layout 的 `happen` / `link` / `note` / `vignette`；`note` 可扩至约 60–120 字；需要时加 `verse_excerpt`。
...
~~**焦虑**、**悲伤** 发现专题页固定短句~~ — 人生主题已删（2026-09-16）；应用模式仍禁止医疗/法律/政治指令（见小爱安全规则）。
...
| **搜索**（圣经内） | 找哪节 / 哪段知识脉络？ | 经文结果、历史、知识专题（地图/关系/时间线/示意） |
...
> **实现面（2026-08-05）：** 本章能力的 **唯一产品实现 = Web IM**；安卓通过 Flutter **嵌 H5**（§24），禁止 Flutter 原生气泡/会话列表作为第二真相。
...
发现（消息主屏）
├── 会话列表（默认）   右上：＋（新建群 / 加好友）
└── 通讯录 → /discover/contacts（二级页；含好友与申请）
```

> **相对早期「双 Tab 同屏」：** Web 以消息为主屏、通讯录为二级，能力等价；文档与实现对齐此项。

**消息页结构（顶区极简）：**

```
┌─────────────────────────────────┐
│  消息（通讯录入口）        [＋]   │  ← 无问候、无今日共读条；通讯录进二级
├─────────────────────────────────┤
│  会话列表（竖向 soft 行）         │
│  群 / 单聊 / 新的朋友 / 群通知 …  │
└─────────────────────────────────┘
...
**下拉刷新**（§2.5）：顶橡皮筋刷新会话列表，语义与首页一致；不做底 overscroll。
...
| 多端一致 | **同一套 Web IM** 在 iOS PWA 与 Android Flutter 壳内对齐（§24）；禁止双实现漂移 | P0（随 §23 + §24） |

---

## 24. 双端交付 · iOS PWA × Android Flutter × H5 白名单（2026-08-05 定稿）

> **效力：** 交付形态、模块归属、H5 白名单、Shorebird、差异白名单与分期 **仅以本章为准**（覆盖 2026-08-04 旧稿）。  
> 社交业务规则仍以 **§23** 为准。

### 24.1 产品一句话

> **iOS = Safari PWA（全站 Web）**；**Android = Flutter 安装包**。  
> **发现/IM、活动/运营、部分我的二级、协议/帮助/反馈、部分设置 = H5 白名单**（与 iOS 同一套 Web）。  
> **首页、圣经读经、小爱、我的主屏 = Flutter 原生**（与 PWA **事对齐**，UI 可分端）。  
> **Shorebird = 壳/原生热修，不是业务发布平台。**

### 24.2 一致性定义

| 级别 | 含义 | 门禁 |
|------|------|------|
| **P0 硬** | 用户能完成的事一致；账号/好友/群/消息/进度互通 | 一端缺主路径 → 不得标完成 |
| **P1 H5 面** | IM 等白名单与 iOS **同套 Web UI/操作** | 禁止安卓再造 Flutter IM |
| **P2 原生面** | 读经/小爱/首页 UI 可分端，主路径步骤对齐 | 对照 §24.5 脚本 |
| **P3 系统** | 安装、推送、闹钟机制可不同 | §24.4 白名单；不得造成主路径永久缺口 |

### 24.3 模块归属

| 模块 | iOS PWA | Android | 规则 |
|------|---------|---------|------|
| **发现 / IM** | Web | **嵌 H5** | 唯一 Web 实现；禁止 Flutter IM |
| **活动 / 运营落地** | Web | **嵌 H5** | path 进白名单表 |
| **协议 / 帮助 / 反馈** | Web | **嵌 H5** | — |
| **部分我的二级 / 设置** | Web | **白名单内嵌 H5** | 开关类可原生 |
| **小爱** | Web | **Flutter 原生** | 会话/额度/引用以后端为准；跨容器深链见 §24.6 |
| **圣经读经** | Web | **Flutter 原生** | 边读→半屏→问小爱；勿扰 |
| **首页 / 我的主屏** | Web | **Flutter 原生** | 活动卡可点进 H5；我的禁九宫格任务流 |
| **提醒 / 推送 / 深链** | Web Push 等 | 原生 + 打开 H5 path | — |
| **安装更新** | 加主屏幕 | 官网 Flutter APK；Shorebird 壳热修；整包独立 | 三轨见 §24.7 |

**H5 路径白名单（初版，工程以代码表为准）：**

- `/discover` 及子路径（IM）  
- 活动/运营落地路由（如 `/campaign*`、规格内活动 path）  
- 协议/隐私/帮助/反馈相关 path  
- 产品点名的「我的」二级与说明型设置 path  

白名单外 **默认 Flutter**；新增 path 须改本章与代码表。

### 24.4 系统能力差异白名单

| 允许差异 | 禁止由此推出 |
|----------|----------------|
| 安装 APK vs 加主屏幕；Shorebird 仅安卓 | 业务只在安卓、iOS 永久没有 |
| 推送通道强弱不同 | 圣经 Tab 弹社交；发现底栏角标 |
| 提醒准点安卓更强 | 默认策略一端「强制开」guilt |
| 返回键 vs 左缘滑；键盘适配 | H5 内返回语义不一致（应先关层） |
| 小爱/读经 UI 分端 | 能力一端有一端无 |
...
|---|------|
| 1 | 两端登录同一账号，五 Tab 齐全 |
| 2 | 圣经继续阅读 → 半屏 → 问小爱（安卓跨容器可到小爱） |
| 3 | 发现：会话列表、群/私信收发文字（安卓为 H5） |
| 4 | 经文卡/打卡卡（有群时） |
| 5 | 通讯录/好友申请可达 |
| 6 | 我的主屏 + 设置；协议/帮助可打开 |
| 7 | 活动落地可打开（有活动时） |
...
扩展清单与操作勾选：[`android_dual_end_qa.md`](./android_dual_end_qa.md)。  
旧壳迁移：[`android_legacy_shell_migration.md`](./android_legacy_shell_migration.md)。

### 24.6 Android：H5 容器 · 登录 · 跨容器

| 项 | 定案 |
|----|------|
| **容器** | 白名单内 **System WebView**；发现可常驻；活动/设置按需开 |
| **登录** | 壳登录后 H5 **一次进站已登录**（注入 `presto_session_token`）；登出 **双清** |
| **Origin** | 与 PWA 同站（如 `https://2sc.prestoai.cn`） |
| **返回** | 系统返回先 H5 历史/关半屏，再退 Flutter |
| **深链** | 通知/外链 → Flutter → 白名单 path 开 H5 或原生路由 |
| **读经/群 → 小爱** | 统一锚点语义；跨容器不得丢经节锚点 |
| **主题** | H5 与原生尽量同步纸色/night，减少「两个 App」感 |
| **SLA（发现）** | 返回、键盘、选图、杀进程恢复、弱网诚实 |

### 24.7 发版三轨与 Shorebird

| 轨 | 用途 |
|----|------|
| **web** | IM、活动、协议、帮助、反馈、白名单设置/我的二级；与 iOS 同窗口 |
| **shorebird** | 壳闪退、提醒、推送、安全区、桥、原生读经小修；**禁止** IM/活动业务 |
| **full_apk** | 权限、SDK、签名、大改、min 壳版本强制更新 |

纪律：**能 web 就不 shorebird，能 shorebird 就不 full_apk。**  
需求字段：`surface: h5 | flutter | shell`，`release: web | shorebird | full_apk`。

### 24.8 工程分期

| 阶段 | 目标 |
|------|------|
| D0 | 本章 + 规则 + H5 白名单表落地代码 |
| D1 | Flutter APK 主推；登录桥；发现 H5 可聊 |
| D2 | 活动/协议/帮助/反馈/部分设置 H5；WebView SLA |
| D3 | Shorebird 壳通道 + 门禁 |
| D4 | 深链/推送/跨容器小爱；观测 `client_kind` |
| D5 | 稳态：先 API → Web → 再 Flutter 增强 |

### 24.9 八项持续优化（执行清单）

1. **边界**：白名单表；跨容器小爱深链；首页活动卡契约。  
2. **容器**：发现 WebView SLA；按需打开；弱网诚实。  
3. **登录会话**：Token 注入、登出双清、游客 merge、主题同步。  
4. **三轨发版**：web / shorebird / full_apk 纪律。  
5. **观测门禁**：`pwa` / `android_flutter` / `android_h5_tab`；双端最小脚本。  
6. **中长期**：anchor 协议深化；小爱改回 H5 可评估；Flutter iOS 非本阶段。  
7. **分发**：主推 Flutter APK；旧壳清理；微信逃逸漏斗。  
8. **协作**：H5 一套稿 + 原生一套稿；需求强制 surface/release 字段。

### 24.10 明确不做

- Flutter 再造 IM；Shorebird 发业务；无 Chrome 不可用。  
- 本阶段 Flutter iOS；用 H5 覆盖读经主屏（读经保持原生）。  
- 白名单外随手嵌 Web。
...
| 旧 | 现 |
|----|-----|
| 小爱默认嵌 Web | **小爱 Flutter 原生**；IM 仍 H5 |
| 全站可嵌 Web | **仅 H5 白名单** |
| Chrome Host 主路径 | **Flutter APK + WebView 嵌白名单** |
| Shorebird 未定义 | **壳热修 only** |


## 相关文档

- [阅读体验设计规范](./READING-EXPERIENCE.md) — 字体、版式、对话、专名、对照模式
- [技术架构](./ARCHITECTURE.md) — 双端交付、目录结构、环境变量
- [RAG 与向量库](./RAG.md) — 分块、检索、数据库（参考 minimax_aipodcast）
- [代码实施计划](./IMPLEMENTATION-PLAN.md) — 里程碑、任务清单、Schema、第一周 Checklist；**含「我的」身份页 §1.5 推进**
- [社交 v1.2 计划](./SOCIAL-V12-PLAN.md) — §23 工程批次（实现面服从 §24）
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **§19.14.6 Prompt Writer 脚本对齐** | **2026-09-16** | `scripts/lib/visual_prompt_writer.py`：SYSTEM 对齐定稿；`verse_anchors`/`era_geo`/`negative`；行程跳过总览位图 |
| **§19.14.13 NotebookLM 对齐定稿** | **2026-09-16** | 拍板：经文结构×diagram/多格 UI×AI 填格；半屏原型 `knowledge_page_prototype` |
| **§19.14.17 手稿册总→站** | **2026-09-17** | 拍板：封面卡进全屏；横滑翻页+捏合缩放；返回/系统分享；首页再滑退出专题 |
| **§19.14.16 手绘纸稿气质** | **2026-09-17** | 拍板：复古纸稿+原创动画形象；专题分绘制方案；禁第三方 IP |
| **§19.14.14 信息图闭环** | **2026-09-16** | Explainer 故事脊默认 + 上图下文分享 + `from-scripture` + `?mode=tour` 逐站（现为次要） |
| **§19.14.12 图文分离定稿** | **2026-09-16** | 拍板：干净图+UI讲解条；行程总览 diagram 主路径；分享上图下文 |
| **§19.14.11 可读性定稿 + 样张** | **2026-09-16** | 拍板「讲得清」：版式 A/B、母题≥3、分图叠 happen；重做旷野（总图+6分图）与保罗（总图+4分图）v311 样张 |
| **§19.14.7 图型按场景** | **2026-09-16** | 「总图+分图」改为**常用模式而非硬规定**；按场景出合适图；避免 N 拼 1 与文字墙代替画面 |
| **§19.14.10 中文叠字提升** | **2026-09-16** | 后处理叠字定稿：信息层级 L1–L4、自适应对比+轻 scrim（废止粗白底通栏）、安全区/碰撞避让、烘焙 vs 运行时双通道、`overlay_spec`、叠字质量门 |
| **删除人生主题 + §19.14.8/9** | **2026-09-16** | **删除**盼望/焦虑等人生产品面；搜索改经文+知识专题；认可地图/关系/器物/时间线 → AI 叙事主图（§19.14.8）；新增适合出图的圣经内容扩库清单（§19.14.9） |
| ~~§19.14.7 一次一张主图~~ | 2026-09-16 | ~~单卡一次生图塞全站~~ → **已被「总图+分图」修订取代** |
| **§19.14.7 讲解设计** | **2026-09-16** | 视觉卡不以「一屏一事」限死密度；规定「先讲解设计再画」：叙事弧 + 单站 motif/happen/link；主图靠画面讲故事，文案轻锚；附旷野行程完整讲解脚本 |
| **§19.14 默认 CogView-3-Flash** | **2026-09-16** | 视觉卡建库默认图像模型改为 **`cogview-3-flash`**；`cogView-4-250304` / `glm-image` 为备选对照 |
| **§19.14 免责右下角 + 模型对比** | **2026-09-16** | 新图「释义说明，仅供参考」改叠画面右下角，**不回刷历史样张**；过红海 A/B：`cogview-3-flash` vs `glm-image` |
| **§19.14 原文驱动 + 免责** | **2026-09-16** | 视觉卡每张必叠「释义说明，仅供参考」；DeepSeek Prompt Writer **必须吃 `scripture_text` 原文**，`verse_anchors` 对齐画面，禁止凭印象生图 |
| **§19.14.6 DeepSeek→CogView** | **2026-09-16** | §19.14 增补生图流水线：DeepSeek 据经文简报写高质量 prompt → 智谱 CogView 出图 → 叠字压 WebP；离线建库，不进读经主路径实时双模型 |
| **§19.14 圣经视觉卡** | **2026-09-16** | 新增 §19.14：读经视觉卡内容白/黑名单、交付双档（thumb/full WebP）、性能平衡（云端按需、屏上少解、按屏宽解码、LRU）；§19.6 交叉引用。原则：**库可以多，屏上要少；生成可以大，交付要小** |
| **§5.5 足迹·书架换划线** | **2026-09-04** | 足迹 2×2：**笔记 / 书架 / 成就 / 进行中**；删划线格与主屏独立书架块；划线改由读经选中条 + 笔记库 Tab 承担 |
| **§24 H5 白名单 + Shorebird** | **2026-08-05** | 重写 §24：IM/活动/协议帮助反馈/部分我的与设置 = **H5 白名单**；首页/读经/小爱/我的主屏 = **Flutter 原生**；**Shorebird 仅壳热修**；八项持续优化；三轨发版 |
| **§24 双端交付定稿** | **2026-08-04** | 新增 **§24**：iOS = Safari PWA，Android = Flutter 官网安装包；一致 = 事一致（硬）+ UI/操作尽可能一致（软）；**IM 唯一实现 = Web**（安卓嵌同一套）；小爱默认同 Web；系统差异白名单；安卓容器 **WebView 默认 / TWA 增强**；能力矩阵与 D0–D4 分期。同步 §1.4、§2.6 规则 #13/#13a、§23 实现面、待优化清单、文首摘要；废止「双端皆 Flutter 原生 App / 无 Chrome 不可用」为唯一形态 |
| **v1.0（我的·读经身份页）** | **2026-07-30** | §5.5 定稿为 **读经身份页**：Hero（预设默认头像+可上传、昵称、**纯签名**）→ **同行主卡整卡进报告（无「查看回顾」）** → **足迹 2×2** → **常用≤4**；UI 与首页同构；性能/弱网/隐私护栏；**§5.5.8 P0→P2 推进方案**。覆盖旧「仅预设不可上传」与主屏纵向四卡。文首摘要、§4.1/4.2、待优化清单、§18.5 同步 |
...
| **v1.1（个人资料）** | **2026-06-29** | §5.5 个人资料规则：头像 **全预设**（初始随机、可换；**上传能力见 2026-07-30 定稿修订**）；用户名 **随机生成**（圣经正向词 前缀+后缀，可改/「换一个」）；签名 自定义 **≤15 字**。canvas 同步 |
