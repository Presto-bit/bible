#!/usr/bin/env bash
# 构建彼爱 Chrome-hosted PWA 安装包，并同步到 Web 静态下载目录。
set -euo pipefail
...
data.update({
  ...
  "notes": "Chrome-hosted PWA 安装包：分发用 APK，渲染用 Chrome TWA；非 System WebView。",
})
...
echo "manifest: $ROOT/host-manifest.json → ${VERSION_NAME} (${VERSION_CODE})"
