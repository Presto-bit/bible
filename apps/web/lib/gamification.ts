...
export function computeBadges(opts: {
  ...
  const { streak, readBooks, totalBooks, noteCount = 0 } = opts;
  const ntDone = readBooks >= 27;
  const psaPct = Math.min(100, Math.round((readBooks / Math.max(1, totalBooks)) * 150));
  return [
    {
      id: 'streak7',
      label: '连续7天',
    ...
    },
    {
      ...
      progress: `${Math.min(readBooks, 10)}/10`,
    },
    {
      id: 'nt',
      label: '新约通读',
    ...
    },
    {
      ...
      desc: '答对 50 道知识卡',
      icon: '🃏',
      done: quizCorrectCount() >= 50,
      progress: `${Math.min(quizCorrectCount(), 50)}/50`,
    },
    {
      id: 'psa',
      label: '诗篇旅人',
    ...
    },
    {
      ...
      progress: `${Math.min(aiQuizWins(), 5)}/5`,
    },
  ];
}
