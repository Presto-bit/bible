"""小爱输出场景：统一契约，各入口只选 scene。"""
from __future__ import annotations

from dataclasses import dataclass

# 各场景正文结构（Markdown 三级标题 + 列表/表格）
_MD = (
    "使用标准 Markdown：小节用 ### 中文标题（如 ### 摘要），"
    "列表用 - 或 1. ，不要用【】包裹标题。"
)
# 篇幅为建议区间：写完整即可收束，单节宜精炼，多节可适度加长
_LEN_HINT = (
    "各小节下必须用 - 列表输出要点，禁止连续超过 2 句的散文段；"
    "每条须是一个完整观点或层次，禁止为凑字数把一句拆成多条；"
    "须一次性写完全部规定 ### 小节与足够要点后再停笔，不要留半成品；"
    "写满要点即停，不要重复、堆砌；句子自然收束，不要中途截断。"
)
_BULLETS = "每条要点单独一行，以「- 」开头（≤55 字/条）。"
_BULLETS_RICH = (
    "每条 1–2 句完整判断（约 50–80 字），须点出经节范围或关键词；"
    "禁止只写名词短语或标题式短句。"
)
_BG_CONTEXT = (
    "写清**上下文**：本段在书卷/章节中的位置、与前文后文的衔接、当时读者处境；"
    "为背景铺垫，不在此节展开经句详解。"
)
_EXPLAIN_CONTEXT = (
    "承接「经文背景」与（若有）「段落脉络」，按序覆盖："
    "①上下文衔接 ②当时原意（关键字句） ③写作对象 ④写作意图；"
    "用「在……背景下/语境下……」写法，不重复背景已写的事实信息。"
)
_EXPLAIN_COMPACT = (
    "2 句 prose：句 1 = ②原意 + ①衔接；句 2 = ③对象或 ④意图（至少其一）。"
)


@dataclass(frozen=True)
class SceneSpec:
    id: str
    mode: str
    label: str
    max_tokens: int
    wants_followups: bool
    format_guide: str
    use_rag: bool = True


SCENES: dict[str, SceneSpec] = {
    "verse_quick": SceneSpec(
        id="verse_quick",
        mode="explain",
        label="快读解释",
        max_tokens=650,
        wants_followups=False,
        format_guide=(
            f"{_MD}\n"
            "OIA 四节顺序固定（半屏快懂）：\n"
            "### 摘要\n"
            "观察：1 句完整话（≤55 字），以「这节/本章」或主语起笔；主谓完整、以。收束，勿残缺起笔。\n"
            "### 经文背景\n"
            "背景：1 句，当时处境或与前后文/同卷主线的衔接。\n"
            "### 经文解释\n"
            f"解释：{_EXPLAIN_COMPACT}\n"
            "### 今日回应\n"
            "应用：1 句，温柔贴近生活的回应。\n"
            "建议篇幅约 240–320 字；每节 prose 为主，不要列表堆砌。"
            "不要「相关追问」。"
        ),
    ),
    "verse_full": SceneSpec(
        id="verse_full",
        mode="explain",
        label="综合解读",
        max_tokens=900,
        wants_followups=False,
        format_guide=(
            f"{_MD}\n"
            "OIA 四节（Tab 深读默认；半屏由 depth 压缩为每节 1 句）：\n"
            "### 摘要\n"
            "1–2 句完整话，直接说核心。\n"
            "### 经文背景\n"
            f"2–3 条，{_BULLETS}处境/前后文/同卷主线，勿重复摘要。\n"
            "### 经文解释\n"
            f"2–3 条完整句，{_BULLETS}{_EXPLAIN_CONTEXT}\n"
            "### 今日回应\n"
            f"1 条核心 + 2 条行动或默想，{_BULLETS}\n"
            "建议篇幅约 450–650 字。不要「相关追问」。"
        ),
    ),
    "chat_explain": SceneSpec(
        id="chat_explain",
        mode="explain",
        label="释经解释",
        max_tokens=1024,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 摘要\n"
            "1–2 句完整话。\n"
            "### 经文背景\n"
            f"2–3 条，{_BULLETS}处境/前后文/同卷主线。\n"
            "### 经文解释\n"
            f"2–3 条，{_BULLETS}{_EXPLAIN_CONTEXT}\n"
            "### 今日回应\n"
            f"1 条核心 + 2 条行动，{_BULLETS}\n"
            f"建议篇幅约 450–550 字。{_LEN_HINT}"
        ),
    ),
    "chat_understand": SceneSpec(
        id="chat_understand",
        mode="understand",
        label="理解默想",
        max_tokens=1024,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 摘要\n"
            "1 句。\n"
            "### 经文要旨\n"
            f"2–3 条，{_BULLETS}\n"
            "### 默想引导\n"
            f"2–3 条，{_BULLETS}温柔连接生命。\n"
            f"建议篇幅约 300–420 字。{_LEN_HINT}"
        ),
    ),
    "chat_apply": SceneSpec(
        id="chat_apply",
        mode="apply",
        label="生活应用",
        max_tokens=900,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 摘要\n"
            "1 句。\n"
            "### 核心提醒\n"
            f"1–2 条，{_BULLETS}\n"
            "### 具体行动\n"
            f"2–4 条，{_BULLETS}贴近日常。\n"
            f"建议篇幅约 280–380 字。{_LEN_HINT}"
        ),
    ),
    "chat_study": SceneSpec(
        id="chat_study",
        mode="understand",
        label="预备查经",
        max_tokens=1400,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 经文分段\n"
            "2–4 个自然段，各 1 句概括（- 列表）。\n"
            "### 背景要点\n"
            "3–6 句。\n"
            "### 讨论问题\n"
            "6 个（观察 2、解释 2、应用 2），用 - 列表。\n"
            "### 带领提示\n"
            "2 条（- 列表）。\n"
            f"建议篇幅约 400–700 字。{_LEN_HINT}"
        ),
    ),
    "chat_preach": SceneSpec(
        id="chat_preach",
        mode="preach",
        label="讲道大纲",
        max_tokens=1400,
        wants_followups=False,
        format_guide=(
            f"{_MD}\n"
            "### 主题句\n"
            "1 句（15–25 字）。\n"
            "### 经文重述\n"
            "2–3 句。\n"
            "### 大纲\n"
            "3 个论点（每点：小标题 + 经文依据 + 展开 1–2 句，可用有序列表）。\n"
            "### 例证建议\n"
            "每点 1 个生活化方向（- 列表）。\n"
            "### 结论与回应\n"
            "1 个具体回应呼召。\n"
            f"不要输出「相关追问」。{_LEN_HINT}"
        ),
    ),
    "chat_compare": SceneSpec(
        id="chat_compare",
        mode="compare",
        label="译本对照",
        max_tokens=1100,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 一句话\n"
            "不超过 25 字，说明这节在讲什么；不要术语。\n"
            "### 读起来哪里不一样\n"
            "只对比读者上下文里提供的两本译本（若只有一本则谈常见译法）。"
            "每本摘一句短引文，用白话写「差别在于…」。不要用表格，不要大段抄经。\n"
            "### 原文帮你抓重点\n"
            "最多 1～2 个关键词：中文意思 → 为何影响翻译；可附音译。"
            "不要 Strong 编号，不要逐词罗列。无把握则省略本段。\n"
            "### 可以怎么读\n"
            "2～3 句读经提示或提醒。\n"
            f"建议篇幅约 180–320 字。{_LEN_HINT}"
        ),
    ),
    "chat_original": SceneSpec(
        id="chat_original",
        mode="compare",
        label="原文释义",
        max_tokens=1100,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 一句话\n"
            "不超过 25 字，说明这节在讲什么；不要术语。\n"
            "### 读起来哪里不一样\n"
            "只对比读者上下文里提供的两本译本（若只有一本则谈常见译法）。"
            "每本摘一句短引文，用白话写「差别在于…」。不要用表格，不要大段抄经。\n"
            "### 原文帮你抓重点\n"
            "最多 1～2 个关键词：中文意思 → 为何影响翻译；可附音译。"
            "不要 Strong 编号，不要逐词罗列。无把握则省略本段。\n"
            "### 可以怎么读\n"
            "2～3 句读经提示或提醒。\n"
            f"建议篇幅约 180–320 字。{_LEN_HINT}"
        ),
    ),
    "chat_general": SceneSpec(
        id="chat_general",
        mode="explain",
        label="主题问答",
        max_tokens=1600,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "### 摘要\n"
            "1 句（≤40 字），直接点明问题的核心答案。\n"
            "### 时间线\n"
            "人物/历史类问题须写 3–6 条，格式「- **年代** 事件」（年代可写「约前930年」「约30年」等）。"
            "非历史题可省略本小节。\n"
            "### 正文\n"
            "2–4 个自然段，用清楚口语直接回答读者问题"
            "（人物生平按时间线、教义题按要点、历史题按事实链），"
            "不要套用「经文要旨」「默想引导」「经文解释」等面向单段经文的栏目。\n"
            "### 相关经节\n"
            "2–3 条（- 列表），格式「书卷名 章:节 — 为何值得读」。\n"
            f"建议篇幅约 380–720 字。{_LEN_HINT}"
        ),
    ),
    "chat_viewpoints": SceneSpec(
        id="chat_viewpoints",
        mode="explain",
        label="并列观点",
        max_tokens=1800,
        wants_followups=True,
        format_guide=(
            f"{_MD}\n"
            "本题若存在多种常见理解，请并列呈现；不要下「正确的是 A」式裁决。\n"
            "开篇先用 1 句免责声明（可放入引用块）："
            "「本题存在多种理解，以下并列供参考；请以圣经原文为准。」\n"
            "固定结构（默认 2 个观点，最多 3 个）：\n"
            "### 观点 A · 简述（一句话）\n"
            "- 主要依据：经文… / 资料脚注\n"
            "- 强调什么：…\n"
            "### 观点 B · 简述（一句话）\n"
            "- 主要依据：经文… / 资料脚注\n"
            "- 强调什么：…\n"
            "### 共同点\n"
            "1–2 句，写双方都承认的内容；若确实没有可省略本小节。\n"
            "### 建议你怎么读\n"
            "1–2 句：回去看哪几节、注意什么语境；不替用户做教义裁决。\n"
            "约束：每个观点尽量挂脚注；资料不足时写「本次资料不足，仅作常见分类说明」，"
            "禁止伪造注释来源。可用「传统上较多教会持 A」这类描述性表述（需有据）。\n"
            "相关追问优先给：「只展开观点 A」「双方经文对照」「这对今日应用意味着什么」。\n"
            f"建议篇幅约 360–640 字。{_LEN_HINT}"
        ),
    ),
    "summary_chapter": SceneSpec(
        id="summary_chapter",
        mode="explain",
        label="章导读",
        max_tokens=1400,
        wants_followups=False,
        use_rag=False,
        format_guide=(
            f"{_MD}\n"
            "### 本章概览\n"
            "1 句（≤40 字），点明本章在主脉中的位置与要旨。\n"
            "### 核心内容\n"
            "3–5 条短句（- 列表），概括主要情节或教导要点；长章可写到 6 条。\n"
            f"建议篇幅：短章（≤20 节）约 120–220 字；较长章约 220–360 字。"
            f"适合读经前快读导读，通顺口语化。不要输出「相关追问」。{_LEN_HINT}"
        ),
    ),
    "summary_chapter_outline": SceneSpec(
        id="summary_chapter_outline",
        mode="explain",
        label="章结构导读",
        max_tokens=1600,
        wants_followups=False,
        use_rag=False,
        format_guide=(
            f"{_MD}\n"
            "本章节数较多，请按自然段划分，不要逐节罗列。\n"
            "### 本章概览\n"
            "1 句（≤40 字），点明本章要旨。\n"
            "### 分段要点\n"
            "3–5 个小段，每段用 ### 小标题（4–8 字）+ 1–2 句，或 - 列表项。\n"
            "### 读经提示\n"
            "1–2 句，点出 1–2 处重点节或阅读顺序。\n"
            f"建议篇幅约 280–480 字。不要输出「相关追问」。{_LEN_HINT}"
        ),
    ),
    "summary_book": SceneSpec(
        id="summary_book",
        mode="explain",
        label="卷导读",
        max_tokens=1200,
        wants_followups=False,
        use_rag=False,
        format_guide=(
            f"{_MD}\n"
            "### 卷概览\n"
            "1–2 句（≤60 字），点明整卷在圣经主脉中的位置与要旨。\n"
            "### 结构脉络\n"
            "3–4 段短段落（不用逐章罗列），按书卷自然结构划分（如创造—堕落—族长、"
            "出埃及—西奈—旷野等），每段 1–2 句说明该段主题与转折。\n"
            "### 核心主题\n"
            "3–4 条短句（- 列表），提炼整卷反复出现的神学主题或关键教导。\n"
            "### 读经提示\n"
            "1–2 句，点出 2–3 处重点段落或钥节（可标章節），帮助读者把握全书重心。\n"
            f"建议篇幅约 220–380 字。不要逐章概述，不要输出「相关追问」。"
            f"适合读经前快读导读，通顺口语化。{_LEN_HINT}"
        ),
    ),
}

# 指定 surface 时强制关闭 RAG（如首页 rail 预填，仅首问轻量作答）
NO_RAG_SURFACES: frozenset[str] = frozenset({"home_prefill"})
# 无经文锚点时，这些 scene 会降级为 chat_general
REF_BOUND_SCENES: frozenset[str] = frozenset(
    {
        "verse_quick",
        "verse_full",
        "chat_explain",
        "chat_understand",
        "chat_apply",
        "chat_study",
        "chat_preach",
        "chat_compare",
        "chat_original",
    }
)
MODE_TO_SCENE: dict[str, str] = {
    "explain": "chat_explain",
    "understand": "chat_understand",
    "apply": "chat_apply",
    "compare": "chat_compare",
    "original": "chat_original",
    "preach": "chat_preach",
}


def verse_scene_format_guide(
    scene_id: str,
    verse_span: int = 1,
    *,
    depth: "DepthProfile | None" = None,
) -> str:
    """释经：按选区节数 / depth 动态输出 OIA 格式指引。"""
    from .depth_router import DepthProfile

    spec = SCENES.get(scene_id)
    if not spec or scene_id not in ("verse_full", "verse_quick"):
        return spec.format_guide if spec else ""
    span = max(1, int(verse_span or 1))
    if depth and depth.depth == "oia_compact":
        lo, hi = depth.target_chars - 40, depth.soft_max
        return (
            f"{_MD}\n"
            f"共 {span} 节：按主题归纳，不要逐节罗列。\n"
            "OIA 四节各 1–2 句 prose：### 摘要 → ### 经文背景 → ### 经文解释 → ### 今日回应。\n"
            f"经文解释：{_EXPLAIN_COMPACT}\n"
            f"建议篇幅约 {lo}–{hi} 字。不要「相关追问」。"
        )
    if depth and depth.depth in ("oia_standard", "oia_deep"):
        titles = " → ".join(f"### {t}" for t in depth.sections)
        return (
            f"{_MD}\n"
            f"共 {span} 节：按主题归纳，禁止逐节 OIA。\n"
            f"顺序：{titles}。\n"
            f"建议篇幅约 {depth.target_chars}–{depth.soft_max} 字。不要「相关追问」。"
        )
    if depth and depth.depth == "deep":
        return (
            f"{_MD}\n"
            f"共 {span} 节连续经文：先抓整段主线，**禁止逐节解释**。\n"
            "### 摘要\n"
            "1 句（≤50 字）。\n"
            "### 经文背景\n"
            f"2 条，{_BULLETS_RICH}{_BG_CONTEXT}\n"
            "### 段落脉络\n"
            f"3–4 条，{_BULLETS_RICH}\n"
            "### 经文解释\n"
            f"4–5 条，{_BULLETS_RICH}{_EXPLAIN_CONTEXT}\n"
            f"建议篇幅约 {depth.target_chars}–{depth.soft_max} 字。{_LEN_HINT}"
        )
    if span <= 2:
        return spec.format_guide
    lo, hi = (280, 360) if span <= 5 else (320, 440)
    return (
        f"{_MD}\n"
        f"共 {span} 节经文：按主题归纳，不要逐节罗列。\n"
        "OIA 四节：### 摘要 → ### 经文背景 → ### 经文解释 → ### 今日回应。\n"
        f"经文解释：{_EXPLAIN_CONTEXT}\n"
        f"建议篇幅约 {lo}–{hi} 字。不要「相关追问」。"
    )


def resolve_scene(scene: str | None, mode: str, *, has_ref: bool = True) -> SceneSpec:
    if not has_ref:
        if scene and scene in SCENES and scene not in REF_BOUND_SCENES:
            return SCENES[scene]
        return SCENES["chat_general"]
    if scene and scene in SCENES:
        return SCENES[scene]
    mapped = MODE_TO_SCENE.get(mode)
    if mapped and mapped in SCENES:
        return SCENES[mapped]
    return SCENES["chat_explain"]
