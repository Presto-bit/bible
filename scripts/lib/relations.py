...
# parent 边 label 描述 from（父母）；spouse 边 label 描述 to（配偶角色）
...
def peer_relation_label(rel: dict, center_id: str) -> str:
    ...
    if typ == "parent":
        if direction == "in":
            return raw or DEFAULT_LABELS["parent"]
    ...
    if typ == "mentor":
        if direction == "in":
            return raw or "导师"
        return raw or DEFAULT_LABELS["mentor"]

    return raw or DEFAULT_LABELS.get(typ, typ)
