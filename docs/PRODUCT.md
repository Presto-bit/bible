...
- **唯一翻页手势：左右滑动**；页指示可用极简圆点/页码（可选，不抢画面），**不做**站序号条、**不做**「第 N 站」Chip/底栏列表入口。  
- **边界：** 第一页再右滑无效；**最后一页再左滑无效**（弹回，不循环、不接广告式下一专题）。  
- 打开专题默认停在 **总手稿**；记住上次页码可选（非必须）。  
- 每页可 pinch / 保存 / 分享（单页带走）；整册分享非 MVP。

##### 页面简洁（明确不做）

- 站手稿 / 翻页容器 **不**放「读这段经文」「问小爱」等次要 CTA（读经与小爱仍由产品其他入口承担，不挤手稿阅读）。  
- **不**做站间进度 guilt、下一站预告条、无限下滑 feed。  
- **不**另开第三套 `?mode=tour` 导览 UI；旧逐站模式收敛进本手稿册或废弃。
...
| **§19.14.17 手稿册总→站** | **2026-09-17** | 拍板：1 总 + N 站横滑翻页（末页不可再滑）；无站序号条；页面不加读经/问小爱 |
